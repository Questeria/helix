...
[hand-encoded bytes]
   ...
   M2-Planet  ── tiny C subset compiler (vendored at this point)
       │
       ▼
   kov-libc + kovc-bootstrap  ── written in M2 C-subset
       │
       ▼
   kovc  ── self-hosted in Kov
```
...
- **stage0/kov-libc/** — written by us in M2-Planet C-subset. Avoids GPL-3.0 contagion. ~200–500 LOC.
...
- Eventual `kovrt` runtime: Windows-native (CUDA Driver API → RTX 5090). Bootstrap and runtime can target different OSes.

## License

GPL-3.0 sources (vendored stage0-posix / mescc-tools / M2-Planet) are kept separately from kov-libc/kovc-bootstrap which are Apache 2.0. The two trees are statically separable.
