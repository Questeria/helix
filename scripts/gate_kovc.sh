...
if [ "$FIX_OK" = "1" ]; then
  ...
  # v1.5 S0 re-mint (2026-06-13): 0992dddd... -> dffd778c... -- the additive ternary type t2 (tag 12;
  # parser.hx 3 resolvers + reverse-name) moves the compiler self-image. The 3-way fixpoint K2==K3==K4
...
# Increment 1 = the type registers end-to-end and ternary values round-trip + accumulate;
# scalar domain i32 (the GPU integer-accumulate ternary matmul is S0 increment 2). trit_dot
# exercises ALL 3 type-ident resolvers (make_trit t2-return + dot4 t2-params + (x as t2) cast)
# + the reverse-name, with a RUNTIME ternary dot [+1,-1,0,+1].[10,20,30,5] = -5 -> 42 sentinel.
# NO kovc.hx GPU/PTX change -> vector_add/tiled .ref.ptx byte-identical; self-host source uses
# no t2 -> K2==K3==K4 byte-identical (the pinned fixpoint SHA MOVES, re-minted with this reason).
chk "$GENC/trit_dot.hx" 42
# v1.5 S0 (2026-06-13): +2 more ternary corpus rows (DoD S0 requires >=3 ternary rows).
# trit_roundtrip: each of {-1,0,+1} round-trips t2<->i32 (identity) + a +/0/- weighted sum -> 42.
...
# v1.5 S0 corpus floor bump 109->110 (2026-06-13): +1 row trit_dot.hx -- first-class ternary
# type t2 (tag 12, BitNet b1.58, -1/0/+1; scalar domain i32). NO GPU/PTX change -> vector_add/
# tiled .ref.ptx byte-identical; self-host source uses no t2 -> K2==K3==K4 byte-identical (pinned
# EXPECT_FIX above re-minted with this reason). GPU int-accumulate ternary matmul = S0 increment 2.
# v1.5 S0 corpus floor bump 110->112 (2026-06-13): +2 ternary rows (trit_roundtrip, trit_matmul_cpu)
