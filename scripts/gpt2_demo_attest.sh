...
window.DEMO_REPORT = {
  ...
  gates: [
    ...
    { name: "BYTE-IDENTICAL", verdict: "PASS", key: "two runs, same to the byte",
      ...
      hash: "$RUN1_SHA" },
    { name: "CPU NO-PTXAS BLOCK-0 PARITY", verdict: "PASS", key: "argmax 262 == oracle", flag: "NO PTXAS / PUREST TRUST",
      desc: "Same model on a CPU path with NO ptxas/GPU boundary at all; block-0 max-abs 1.144e-04; argmax 262 == oracle; greedy gen token-for-token == oracle." },
  ...
  paths: [
    ...
    { kind: "cpu", title: "CPU no-ptxas path", badge: "PUREST TRUST",
      body: "ALL arithmetic in kovc-compiled-from-raw Helix. ZERO trusted component above the 299-byte seed. ~130 sec/token — slow by design: the product is verifiability, not speed.",
      stats: [ { k: "trusted above seed", v: "ZERO" }, { k: "throughput", v: "~130 s / token" } ] }
  ],
  fences: [
    { k: "exactly", v: "1 committed .py" },
    { k: "26 committed .c/.h", v: "24 from-raw UNCHANGED + 2 demo launchers" },
    { k: "corpus", v: "109 / 0" },
