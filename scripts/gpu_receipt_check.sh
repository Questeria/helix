...
# nc <label> <receiptpath> <expected-REJECT-cause> : the receipt MUST be rejected, AND by the named check
# (so the per-NC attribution -- e.g. "NC1 isolates the Freivalds teeth" -- is PINNED against regression).
nc(){ local out v c; out=$("$CL" x x 0 receipt_check "$2" 2>&1)
  v=$(printf '%s\n' "$out" | sed -n 's/.*-> \(RECEIPT_PASS\|RECEIPT_FAIL\)$/\1/p' | tail -1)
  c=$(printf '%s\n' "$out" | sed -n 's/.*REJECT=\([A-Z0-9_]*\).*/\1/p' | tail -1)
  if [ "$v" = RECEIPT_FAIL ] && [ "$c" = "$3" ]; then say "    [$1] RECEIPT_FAIL via REJECT=$c (expected $3)  OK"
  elif [ "$v" = RECEIPT_FAIL ]; then bad "[$1] rejected but REJECT=$c, expected $3 (attribution drift)"
  else bad "[$1] receipt_check=$v but expected RECEIPT_FAIL"; fi; }
...
if [ ! -s /tmp/rc_nc1.txt ]; then bad "NC1 receipt not emitted"; else
  if [ "$(grep '^H_C=' /tmp/rc_nc1.txt)" = "$GEN_HC" ]; then bad "NC1 vacuous -- mutate did not change C (H_C identical to genuine)"; else nc "NC1 forged-output" /tmp/rc_nc1.txt CHECK2; fi
fi
...
if [ "$(grep '^H_C=' /tmp/rc_nc2.txt)" = "$GEN_HC" ]; then bad "NC2 vacuous -- H_C flip did not change the line"; else nc "NC2 tampered-hash-bind" /tmp/rc_nc2.txt CHECK3; fi
...
if grep -q '^round 0 lhs=1 rhs=1' /tmp/rc_nc3.txt; then nc "NC3 echo-not-trusted" /tmp/rc_nc3.txt CHECK2; else bad "NC3 vacuous -- round-echo doctor did not apply"; fi
...
if grep -q '^seed_W=5' /tmp/rc_nc4.txt; then nc "NC4 tampered-weight" /tmp/rc_nc4.txt CHECK1; else bad "NC4 vacuous -- seed_W flip did not apply"; fi
...
nc "NC5 vacuity" /tmp/rc_nc5.txt VACUITY
...
if cmp -s /tmp/rc_badkern.hx "$KERN"; then bad "NC6 vacuous -- drop-term sed did not match the committed kernel"; else
  ...
  if [ ! -s /tmp/out.ptx ]; then bad "NC6 corrupted kernel emitted no PTX"; else
    ...
    if [ ! -s /tmp/rc_nc6.txt ]; then bad "NC6 receipt not emitted"; else
      if [ "$(grep '^H_C=' /tmp/rc_nc6.txt)" = "$GEN_HC" ]; then bad "NC6 vacuous -- corrupted kernel produced the SAME C as genuine"; else nc "NC6 kernel-corruption" /tmp/rc_nc6.txt CHECK2; fi
    fi
  fi
fi
