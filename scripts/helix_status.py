...
K_BOOTSTRAP_CHUNKS_DONE = 240      # last bump: K1.F23 -- DEFECT DISCOVERY: probe A confirms __arena_push/__arena_set/__arena_get round-trip works in main() (rc=7); probe B reveals that moving the __arena_push into a HELPER fn produces a K2 binary that SIGILLs at runtime (rc=132). Blocks the user-fn-based tile_zeros approach until fixed (K1.F23b). Fallback: implement __tile_zeros as an inline builtin (K1.F23c) -- cursor-bump trick. Pinned via test_bootstrap_kovc_k1f23_arena_helper_fn_known_broken_self_host (asserts current rc=132; flip to rc=7 when fixed). K_BOOTSTRAP_TILE_OPS_PLAN.md updated with the finding + revised chunk sequence
