...
K_BOOTSTRAP_CHUNKS_DONE = 432      # last bump: STAGE-0 LADDER rung 4 -- catm DONE. Vendored oriansj/stage0-posix-amd64 catm_AMD64.hex2 (GPL-3.0, pinned 15535f8, src sha 55f65975) + built with OUR hex2 (../hex2/hex2.bin catm_AMD64.hex2 catm.bin -> 299B sha 911d19bf; no assembler, no pre-built binary). catm = file-concatenation tool (catm OUT in1..inN), the glue for prepending the ELF header to each program from M0 up. REPRODUCIBLE (sha stable 2 builds) + 2/2 tests under WSL (concat order + binary-safe incl NUL). Trust chain hex0(frozen)->hex1->hex2->catm. Next rung: M0 (catm builds M0.hex2 = ELF-amd64.hex2 + M0_AMD64.hex2, hex2 assembles -> M0 the macro assembler), then cc_amd64, M2-Planet.
