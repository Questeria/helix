...
K_BOOTSTRAP_CHUNKS_DONE = 253      # last bump: K1.F25 -- shipped __tile_sub elementwise subtract builtin (slot 176). Codegen is a 1-byte op-flip of __tile_add (sub opcode 2B vs add opcode 03; identical SIB encoding, byte count, rel8 displacements). Single-cell verified: a[0]=20, b[0]=7 -> dst[0]=13 (3/3 runs). Multi-cell verified with values < 256 to dodge a NEWLY-SURFACED pre-existing bootstrap defect: __arena_get(x) results, when used in a sum that overflows u8, get narrowed to u8. Sum 50+100+150+200=500 returns 244=500%256; individual reads return correct values. K1.F25's codegen is fine; the defect is in expression-type inference. Pre-existing defect documented for next chunk (K1.F26). Tile ops Phase-0: __tile_zeros + __tile_add + __tile_sub now REAL (3 of 5). Remaining phase-0: __tile_mul (imul opcode shifts displacements), __tile_matmul
