...
K_BOOTSTRAP_CHUNKS_DONE = 305      # last bump: K2.AF -- trap-id capture helper (_kovc_self_host_compile_and_run_full). Variant of the existing helper that returns (rc, stdout, stderr) instead of just rc. Speeds up codegen-debugging multi-tick arcs (K1.F5g2+ struct-return, K3.* audit families) by letting probes see the bootstrap's panic trap-id and message directly. Bootstrap's panic mechanism writes "panic[id]: message\\n" to stderr before ud2-ing; capturing stderr maps directly to the bad emit site via kovc.hx's trap-id reservation comments. DEMO FINDING: struct-return gap (the K1.F5g2 blocker) shows EMPTY stderr on rc=132, meaning it's NOT a panic trap -- it's a raw ud2 or memory fault. That's a localizing finding for K1.F5g2+ work. Speed-up #2 from the strategic-discussion list.
