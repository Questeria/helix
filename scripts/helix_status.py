...
K_BOOTSTRAP_CHUNKS_DONE = 256      # last bump: K2.W -- doc-sync chunk. Updated docs/V3_HANDOFF.md section 4b state-at-handoff to reflect chunks-done counter 255 (was 219, ~36 chunks stale), the K1.F22 macro-expansion family closure (panic!/print!/println!/eprint!/eprintln!/todo!/unimplemented!/unreachable!/assert!/assert_eq!/assert_ne!), the TILE OPS PHASE-0 ROW CLOSURE (K1.F23c through K1.F27 -- all 5 tile ops real, byte counts and slot assignments recorded), and the two major false-positive corrections this session: K1.F24i (WSL printf-multiline-source helper bug) and K1.F26 (Linux kernel exit-code-u8 truncation, NOT a bootstrap defect). Doc-only chunk; no code changes; K2 corpus size test passes
