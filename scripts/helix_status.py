...
K_BOOTSTRAP_CHUNKS_DONE = 251      # last bump: K1.F24i -- MAJOR CORRECTION. The K1.F24g/K1.F24h "3-tile __tile_zeros SIGILL" findings were ENTIRELY false positives caused by a test-helper bug. _kovc_self_host_compile_and_run wrote the source with `printf %s {repr(k2_src)}` -- repr() escapes newlines to backslash-n (two chars) and `printf %s` writes verbatim without expanding escapes, so multi-line sources ended up on disk as ONE LINE with literal `\\n` separators. Lexer choked; validation emitted trap-id 19 from main's prologue; binary SIGILL'd rc=132. Diagnosed by reading raw binary bytes (37 bytes of code ending in `mov eax 0x13; ud2`) AND verifying 3-tile SINGLE-LINE source returns rc=8 correctly 3/3. Helper fixed (stdin pipe via `cat > in_path` with `input=k2_src.encode()` -- preserves all bytes including newlines). Post-fix, all 4 K1.F24g probes return their expected values (8, 5, 10, 0) cleanly 3/3. K1.F24f real-loop investigation deferred to next chunk -- it might also be a false positive. Regression-prevention test added: multiline_source_regression verifies multi-line and single-line forms produce identical results
