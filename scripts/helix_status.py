...
K_BOOTSTRAP_CHUNKS_DONE = 245      # last bump: K1.F24c -- DEFECT EVEN BROADER. Probing user-fn tile_add (no broken-builtin involved, only __arena_get/__arena_set which work standalone from helpers per K1.F23b) ALSO SIGILLs. Narrowed via probe: even a pure `while + let-mut + assign` helper fn (NO ARENA at all) SIGILLs. The K-bootstrap defect surface is much broader than initially thought: helper-fn codegen for {while, let-mut, assign} combos misfires in K2 self-host. K1.F23b probes G+H worked because they had NO loop. The bigger finding: the bootstrap's helper-fn codegen has multiple latent defects masked by lack of K2 self-host coverage for those constructs. Two known-broken pin tests; K1.F24d needs binary-diff against a known-working helper pattern (e.g. K1.F23b probe E's int-param helper)
