...
K_BOOTSTRAP_CHUNKS_DONE = 262      # last bump: K3.T -- partial close of silent-failure-hunter audit HIGH-1. Pre-K3.T, __tile_matmul wrote 4 dst cells unchecked; a bogus dst_off (e.g., 99999999 or any non-tile_zeros value) could corrupt arena cells past the tile (silent miscompile) or write past mapped BSS (SIGSEGV). K3.T adds a 21-byte runtime check at the matmul setup: load dst_off from [rsp], add 4, cmp against helix_arena_cap() (2097152), jbe to skip the trap if dst_off+4 <= cap, else trap with id 27002. Matmul body grows 179 -> 200 bytes. Tests: normal matmul (dst from tile_zeros) returns 5; matmul with dst=99999999 traps with rc=132. NOTE: a_off and b_off READS remain unchecked; the other 3 elementwise tile ops (add/sub/mul) also remain unchecked. K3.U candidate: extend bounds checks to all four ops + all three offsets
