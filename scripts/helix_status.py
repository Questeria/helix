...
K_BOOTSTRAP_CHUNKS_DONE = 423      # last bump: FIXED large-function host STACK OVERFLOW (task 14, was mislabeled corruption). A function's statements form a right-nested AST_SEQ chain; emit_ast_code (kovc.hx 9237) + expr_type (1433, run on every fn body by the body/return-type trap at 9857/9858/9892) each recursed one host-stack frame per statement -> SIGSEGV/empty-ELF at ~290 statements (proven: under ulimit -s unlimited the same source compiled fine). Both converted to iterative right-spine walks (byte-identical output, fixpoint preserved). Push/statement ceiling ~290 -> >=2000. Verified: fixpoint 2 passed; broad parity 403 passed (parity_matrix+stdlib+autodiff); new test_bootstrap_large_main_statement_sequence (1000 pushes) passing, old corruption-xfail removed. REMAINING (task 15): the recursive-descent parser (parse_expr/parse_let) still overflows ~200 flat lets DURING PARSING (tracked by test_bootstrap_large_let_chain_parser_recursion_xfail); walk_for_panic also still recursive.
