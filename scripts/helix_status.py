...
K_BOOTSTRAP_CHUNKS_DONE = 429      # last bump: FIXED the generic-impl-method dispatch bug (task 18). A method on a generic impl block dispatched on a TURBOFISH-constructed instance (let b = Box::<i32>{v:42}; b.get()) miscompiled to SIGILL 132: the turbofish struct literal records the MONOMORPHIZED struct name (Box__i32) on the binding, but the impl method is registered under the ORIGINAL name (Box__get -- generic params type-erased), so dispatch mangled Box__i32__get -> not found -> ud2 -> SIGILL. FIX (parser.hx ~2655, the is_struct_method dispatch branch): truncate the receiver struct name at the first __ mono-separator before mangling, so Box__i32 -> Box -> Box__get. No current struct name contains __ so it is a no-op for all other code. Verified: turbofish-method cases 132->42, controls unchanged; fixpoint 2 passed (K2==K3 byte-identical); broad parity 403 passed (-n auto); the xfail flipped to a passing test_bootstrap_generic_impl_method. Root-caused by a read-only agent + independently verified the SITE (lesson 17). Closes the last real generics/impl-method gap (both buckets now only pending TRAITS).
