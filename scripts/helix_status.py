...
K_BOOTSTRAP_CHUNKS_DONE = 249      # last bump: K1.F24g -- bisect of the multi-builtin SIGILL. 4 progressive probes (P1-P4) each with 3-run K1.F24d retry discipline. SURPRISING FINDING: P1 (3 x __tile_zeros, NO I/O, NO __arena_set/get, NO __tile_add) SIGILLs 3-of-3 deterministically. K1.F23c probe 3 (2 x __tile_zeros, simple subtraction) WORKS reliably (rc=4) so the defect threshold is at 3 successive __tile_zeros calls in the same main() body -- the multi-builtin chain composition is NOT the root cause as originally hypothesized. P2 (2 tiles + __arena_set/get) also SIGILLs 3-of-3, even though K1.F23c probe 2 (1 tile + 2x __arena_set + 2x __arena_get) works. Two narrow defects share root cause: (a) 2 tiles + arena_set/get composition; (b) 3+ tiles alone. Flake-tolerant pin landed; localization investigation deferred to next chunk
