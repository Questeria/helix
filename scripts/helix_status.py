...
K_BOOTSTRAP_CHUNKS_DONE = 424      # last bump: TEST-INFRA T2 -- scaled the Helix-native test runner corpus 8 -> 32 programs (unblocked by the task-14 host-stack-overflow fix; runner main ~1026 stmts vs the new >=2000 ceiling). test_bootstrap_test_runner_over_corpus_subset reproduces pytest pass/fail (all-pass -> 0, one-wrong-expected -> 1) over 32 real CPU-parity programs via the Helix runner: write src -> run_process the fixed-path kovc -> run the ELF -> compare exit code -> tally. Test-only (collect+run): 3 runner tests passed in 78s. Scaling to the full ~720 still needs a loop/manifest runner with a FIXED-size main, which needs a dynamic-path read builtin (read_file_to_arena is compile-time-strlit only) -- T1 follow-up.
