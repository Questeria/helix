...
K_BOOTSTRAP_CHUNKS_DONE = 250      # last bump: K1.F24h -- axis isolation for the 3-tile __tile_zeros SIGILL. Two probes each 3-run: H1 (3 x __tile_zeros(1, 1) with lets + subtract) and H2 (3 x __tile_zeros(2, 2) DISCARDED with no lets). BOTH SIGILL 3/3 -- the bug is NEITHER cell-count / N*M arithmetic (H1 with 1x1 still fails) NOR let-binding slot allocation (H2 with no lets still fails). Bug is inside __tile_zeros codegen ITSELF on the 3rd call. Candidates remaining: (a) 3rd patch_table entry resolution to arena_base; (b) jae/jmp rel8 displacement interference across 3 close-packed bodies; (c) push/pop rdx stack interaction; (d) instruction-encoding alignment. Next chunk: disassemble actual K2 binary bytes for the 3-tile program to localize exactly which instruction sequence trips
