...
K_BOOTSTRAP_CHUNKS_DONE = 428      # last bump: STATUS RECONCILIATION (verify-against-HEAD, lesson 19) -- the 2026-05-28 generics/impl-method bucket notes were STALE. Re-verified at HEAD: bare generic call id(42)/first(42,7), &self receiver, multiple impl blocks, where-clause, and nested generic calls ALL WORK now (old 'pending'/'SIGILL' notes were stale) -- pinned by test_bootstrap_impl_generics_advanced_working + the existing bare_call tests. Found ONE real current bug: generic-impl methods (impl<T> Box<T> method -> SIGILL 132), tracked by test_bootstrap_generic_impl_method_xfail. Reconciled the 2 stale notes. ASSESSMENT: feature work is done/deletion-parity-met; the only remaining GATING work = stage-0 hex0 trust seed (user-steered, weeks) + 5 clean audits. Test-only + status (collect+run: 1 passed / 1 xfailed).
...
PYTHON_DELETION_BUCKETS = [
    ...
    {"name": "Impl-method dispatch (full)",
     "status": "partial",
     "note": "K1.F5b localized fix + K1.M27 probe (2026-05-28): the bootstrap FULLY handles CORE impl-method dispatch -- method on a struct value (p.get()), method with an arg (p.add(2)), and method-calling-method (self.a()+2) -- all compile+run correctly via self-host (test_bootstrap_impl_method_dispatch). FINDING: Python helixc cannot even PARSE the bare `(self)` receiver (ParseError: expected COLON), so the bootstrap EXCEEDS Python here (parallel to the M21 GPU finding) -> these CANNOT be K2-parity entries (Python errors), and DELETION-PARITY for the core case is MET (deleting Python loses nothing). Kept 'partial' pending: generic-impl methods (SIGILL 132, test_bootstrap_generic_impl_method_xfail) + traits. RECONCILED 2026-05-30: &self + multiple-impl-blocks + where-clause + nested-generic re-verified WORKING (test_bootstrap_impl_generics_advanced_working); the old 'pending' for those was stale -- which may themselves be Python-gaps; core dispatch is done + ahead of Python."},
    {"name": "Generic monomorphization (full)",
     "status": "partial",
     "note": "K1.F21 + K1.M28 probe (2026-05-28): Python helixc CANNOT PARSE generic syntax at all (ParseError on `<T>` for fn-generic / generic-struct / turbofish / multi-param), so the bootstrap EXCEEDS Python on generics -> DELETION-PARITY trivially MET (Python supports ZERO generics). Bootstrap: generic-struct (Box<T>=42) + TURBOFISH (id::<i32>(42)=42) WORK + pinned (test_bootstrap_generics_struct_and_turbofish); BUT a BARE generic-fn call without turbofish (id(42), first(42,7)) MISCOMPILES to SIGILL (exit 132) -- the bare-call SIGILL is FIXED (re-verified 2026-05-30 + pinned by test_bootstrap_generics_bare_call); the real remaining gap is GENERIC-IMPL METHODS (impl<T> Box<T> method -> SIGILL 132, test_bootstrap_generic_impl_method_xfail) (a bootstrap-QUALITY gap, NOT a deletion blocker since Python can't do generics). Absolute-completeness: ~4/10 (const-generics/lifetime-only/generic-impl pending; gp-field/where-clauses partial)."},
