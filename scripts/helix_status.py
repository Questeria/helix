...
K_BOOTSTRAP_CHUNKS_DONE = 303      # last bump: K2.AD -- P1.3 follow-up probes for items #6 + #8. #6 generic-fn-calling-generic-fn (`wrap<U>(x) calls id<T>(x)`) WORKS in bootstrap (rc=42); Python ParseError. #8 const-generic (`fn arr<const N: i32>() -> i32`) FAILS bootstrap rc=132 SIGILL; Python ParseError. So P1.3 bucket is now 5/10 DONE (1, 2, 4, 5, 6 -- all bootstrap-only-superset features). 2 PARTIAL (3 gp-field-use, 7 where-clause), 3 PENDING (8 const-gen, 9 lifetime-only, 10 generic-impl). Bucket is MORE THAN HALF CLOSED; remaining items are advanced shapes (const-gen, lifetimes, generic-impl) that likely aren't blocking real-world usage in v1.0. Highest-impact remaining sub-gap is #3 (Stage 28.11 INC-3b gp-field use-sites).
