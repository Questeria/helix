...
K_BOOTSTRAP_CHUNKS_DONE = 431      # last bump: STAGE-0 LADDER rung 3 -- hex2 DONE. Vendored oriansj/stage0-posix-amd64 hex2_AMD64.hex1 (GPL-3.0, pinned 15535f8, source sha d4c011ac) + built it with OUR hex1 (../hex1/hex1.bin hex2_AMD64.hex1 hex2.bin -> hex2.bin 1519B sha 6c69c7e6; argv I/O; no assembler, no pre-built binary). REPRODUCIBLE (sha stable across 2 builds) + 2/2 tests under WSL: plain-hex superset + hex2's long-label/abs-addr FEATURE (assembles the canonical M0 source cat(ELF-amd64.hex2, M0_AMD64.hex2) -> valid 1684-byte M0 ELF, the mescc-tools phase-3 recipe). stage0/hex2/ mirrors the rung pattern. Trust chain: hex0(frozen) -> hex1 -> hex2. Next rung: catm (hex2 builds catm_AMD64.hex2), then M0.
