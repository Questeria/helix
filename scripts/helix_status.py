...
K_BOOTSTRAP_CHUNKS_DONE = 426      # last bump: BIG-STACK ENTRY STUB (task 16, bug #1). Every bootstrap-emitted binary now starts with a 53-byte _start prologue that mmaps a 512MiB stack (MAP_PRIVATE|ANON|NORESERVE, demand-paged so free until touched) and switches rsp before `call main`; fail-safe js-skip on mmap error keeps the kernel stack. The emitted compiler no longer overflows the default 8MB kernel stack on deep recursion. Verified: rec(5000) was rc139/SIGSEGV -> now 7, rec(20000) -> 7, rec(100000 ~411MB) -> 7; 6*7 control -> 42; fixpoint 2 passed (K2==K3 byte-identical -- the stub is deterministic); broad parity 403 passed (-n auto); new test_bootstrap_bigstack_deep_recursion. SUBSUMES task 15 (parser recursion) for practical depths. NOTE: K1 is Python-built so the canonical test_bootstrap_kovc_self_host_loop stays SKIPPED until the matching stub lands in helixc/backend/x86_64.py _start (task 17).
