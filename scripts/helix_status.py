...
K_BOOTSTRAP_CHUNKS_DONE = 275      # last bump: K3.X audit-fix on K1.F31-F40 macro family. Per-batch 3-axis audit ran in parallel (silent-failure-hunter / type-design-analyzer / code-reviewer). 2 axes CLEAN; type-design flagged H1 (detector overlap, but code-reviewer verified semantically disjoint via narrowing predicates) and H2 (aeqii_* name reuse between K1.F31 IDENT/INT and K1.F39 INT/INT, scope-isolated but maintainer-trap). Fixed H2 by renaming F39/F40 locals aeqii->aeqnn, aneii->anenn (n=numeric). Documented H1 + cascade-disjointness-by-narrowing-predicates invariant in a doc-block above the assert family. Behavior preserved (pure rename + comment). M1/M2 (ordering-invariant fragility, magic 20/5) deferred as known-issues. NOT counted toward 5-clean-audit gate since 1 axis non-clean (style only).
