...
K_BOOTSTRAP_CHUNKS_DONE = 293      # last bump: K1.F5c -- extend struct-method dispatch to struct-LITERAL receivers. K1.F5b only handled AST_VAR receivers (`p.method()` where p is a let-bound struct var); K1.F5c handles `P { v: 1 }.method()` where prim is AST_TUPLE_LIT (tag 50, the struct-lit AST). Reads last_struct_idx(sb) side-channel that parse_struct_lit writes; if set, routes through the existing K1.F5b struct-method-call synthesis. Tuple literals (with last_struct_idx==-1) correctly fall through. First Phase-1 P1.2 (impl-method dispatch full) chunk per docs/CATEGORY_2_NEXT_PHASE.md. Test verifies `P{v:32}.add10()` returns 42 via P__add10 routing. K1.F5b regression intact.
