...
K_BOOTSTRAP_CHUNKS_DONE = 261      # last bump: K3.S -- audit-fix MEDIUM-2 from the silent-failure-hunter audit. Pre-K3.S, K1.F28's dbg!(IDENT) shape guard rejected only "true"/"false" (the K3.Q precedent from assert!/assert_eq!). Other 13 reserved keywords lexed as TK_IDENT (type/agent/continue/pub/extern/break/unsafe/try/async/const/union/static/move) silently miscompiled to 0 via the AST_VAR + unbound-name path. K3.S adds a unified is_any_reserved_kw_ident helper (15 nested if-else) and routes dbg's shape guard through it. Reserved-kw forms now cleanly fall through to K1.CB no-op-skip instead of synthesizing AST_VAR(kw_bytes). Tests: dbg!(true);7 returns 7, dbg!(false);11 returns 11, dbg!(x) where x=42 still returns 42. Defense-in-depth fix; observable behavior for reserved-kw operands unchanged (both paths return 0) but the new path is type-safe and documented. Audit HIGH-1 (tile-op arena bounds) still deferred
