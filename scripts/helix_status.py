...
K_BOOTSTRAP_CHUNKS_DONE = 296      # last bump: K1.F5h -- parse-side fix for `&self` receiver in impl-method param. Pre-fix, `impl P { fn read(&self) -> i32 { ... } }` HUNG (parser infinite loop). Root cause: parse_impl_method's param-parser fell through to the `name: T` standard branch on TK_AMP (27), looking for a colon after `&` that doesn't exist. FIX: detect TK_AMP + optional `mut` + `self` IDENT BEFORE the standard name-detection logic. Skip the `&` (and `mut`) tokens so self-detection runs identically to bare `self`. Bootstrap has no refs; pass-by-value is semantically correct. K1.F5h CLOSES: the parse hang. K1.F5h DOES NOT CLOSE: the downstream codegen for self-receiver impl-method calls (bare `self` ALSO returns rc=132 — separate impl-block codegen gap deferred to K1.F5h2). Test verifies the no-hang property (rc != timeout). K1.F5c regression intact.
