...
K_BOOTSTRAP_CHUNKS_DONE = 282      # last bump: K3.Y -- audit-clean signal for K1.F41-F46. PER-BATCH 3-AXIS AUDIT ALL CLEAN: silent-failure-hunter CLEAN (verification matrix proves all 6 arms correct on mac_t4/t5/t6/t7/cur_advance/AST-tag/byte-count axes), type-design-analyzer CLEAN-on-correctness with 4 LOW maintainability suggestions, code-reviewer CLEAN ("ship it" verdict; cross-checked kovc.hx codegen dispatch for AST_LE/GE setle/setge signed boundary semantics and AST_GT signed setg for i32). Adopted finding #3 (partition-table doc) as a no-behavior-change comment above the F41-F46 detectors. Finding #1 (helper extract), #2 (LEN constants), #4 (k+5/k+6 named offsets) deferred as separate cleanup chunks. First fully-clean 3-axis audit since K1.F31-F40 (which was 2-of-3 clean). Stop-criterion counter for 5-consecutive-clean DOES NOT apply yet -- that gate is for end-of-phase 5-axis audits after Python-ready-to-delete.
