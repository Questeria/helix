...
K_BOOTSTRAP_CHUNKS_DONE = 430      # last bump: STAGE-0 LADDER rung 2 -- hex1 DONE. Vendored oriansj/stage0-posix-amd64 hex1_AMD64.hex0 (GPL-3.0, pinned commit 15535f8, source sha 1f53a60c) and BUILT it with OUR hand-authored hex0 (../hex0/hex0.bin decodes the hex1 source -> hex1.bin, 622 bytes; no assembler, no pre-built binary). REPRODUCIBLE (sha c264a2.. stable across 2 builds) + 2/2 tests pass under WSL: plain-hex superset (Hello) + the LABEL feature (hex1 assembles the real upstream hex2 source -> valid 1519-byte ELF). stage0/hex1/ mirrors stage0/hex0/ (source + .bin + .sha256 + build.sh + run_tests.sh + PROVENANCE.md + test/). Trust root unchanged (hex0 FROZEN). Run everything under WSL (lesson 27). Next rung: hex2 (written in hex1 format, built by our hex1).
