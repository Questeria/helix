...
K_BOOTSTRAP_CHUNKS_DONE = 294      # last bump: K2.Z -- impl-method dispatch gap probe (post K1.F5c). Probed 3 remaining P1.2 shapes: (a) chained / non-let-bound receiver `a.chain1().chain2()` -> rc=132 SIGILL, needs fn-name->ret-ty table substrate (~3 chunks K1.F5d-g). (b) &self receiver `impl P { fn read(&self) }` -> hangs / timeout, needs parse-side &self + pass-by-value codegen (~2 chunks K1.F5h-i). (c) Self::make() -> Self -> rc=139 SIGSEGV, needs static-call detection + Self-type resolution (~2 chunks K1.F5j-k). Updated docs/CATEGORY_2_NEXT_PHASE.md P1.2 section with the reordered 10-step chunk plan based on probe results. K1.F5c (struct-literal) confirmed as the only DONE gap. Future ticks pick up K1.F5d next.
