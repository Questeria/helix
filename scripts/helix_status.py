...
K_BOOTSTRAP_CHUNKS_DONE = 308      # last bump: K1.F5j -- CRITICAL AUDIT-FIX. The K2.AH parallel sweep surfaced 18 K1.F* test failures, but they were ACTUAL regressions, not WSL flake. ROOT CAUSE: my K1.F5d/F5e slot allocations at sb+74/76/77 collided with an existing closure-clearing scratch path at parser.hx:9230-9239 (which zeros sb+74..77 per fn-parse), corrupting last_call_ret_struct_idx + fn_ret_struct_tab on every fn-decl and breaking 18 tests silently. RELOCATED: sb+74 -> sb+124; sb+76/77 -> sb+125/126 (verified-free per grep). K1.F31 now passes again post-relocation. The K2.AH sweep ALSO surfaced K1.F23b/F24d/F24g/F26/F27/F9/F14/F20/F22b/F22e/F22k/F30/F31/F33+F34/F43+F44/F45+F46/F47+F48/F51+F52 -- some may have been pre-existing failures, others may be the same slot-collision cascade. Future tick will re-run the sweep to identify residual failures.
