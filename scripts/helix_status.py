...
K_BOOTSTRAP_CHUNKS_DONE = 425      # last bump: SCOPED the K3 trusted seed (gating build-order item) -> docs/K3_TRUSTED_SEED_SCOPING.md. Headline (verified by reading test_self_host_fixpoint.py): the bootstrap ALREADY self-hosts -- K1 (Python-built) self-compiles the full ~1.43MB driver -> K2, and K2==K3 BYTE-IDENTICAL; Python's ONLY trust-chain role is minting K1 via _compile_src_to_elf. The old arena-cap-overflow + read-buffer blockers are CLOSED (arena rescaled to 6291456 slots, read buffer 4MB). THE blocker now = a big-stack ENTRY STUB (bug #1): the emitted compiler overflows the default 8MB stack on full self-compile (deep parser recursion), so it only works under external ulimit -s unlimited and the canonical test_bootstrap_kovc_self_host_loop (test_codegen.py:6725) is SKIPPED. NEXT chunk: emit an _start stub that mmaps a big stack + switches rsp (unblocks unattended self-compile, unskips the canonical test, subsumes task 15). Full stage-0 hex0/Mes/C trust root + DDC = MAJOR deferred (K3-proper, not loop-tractable).
