...
K_BOOTSTRAP_CHUNKS_DONE = 244      # last bump: K1.F24b -- DEFECT BROADER THAN K1.F24 FOUND: probed __arena_push_pair (K1.AF, 2-arg) and __arena_push_triple (K1.AG, 3-arg) via K2 self-host -- BOTH SIGILL even from main scope. The K1.F24 SIGILL is part of a broader pattern: builtins emitting MULTI-CELL CURSOR-BUMP WRITES misfire in K2 self-host. __tile_zeros (cursor bumps but skips per-cell writes via BSS-zero) works fine. __arena_push (1-cell write) works in main but not helpers. Pattern: (multi-cell-write + cursor-bump) is the broken cell. K1.AF/K1.AG likely never had K2 self-host coverage -- only Python helixc compile-and-run. Two known-broken pin tests landed; K1.F24c needs binary-diff investigation of __arena_push vs __arena_push_pair codegen
