...
K_BOOTSTRAP_CHUNKS_DONE = 304      # last bump: K2.AE -- systematic Python-side probe sweep on 8 remaining P1.2/P1.3/P2.1/P2.2 items. 6 of 6 P1.2/P1.3 items (trait dispatch, generic-impl, Type::method, gp-field use-site, where-clause, generic-impl-monomorph) all fail Python at PARSE -- so they're K2-parity-vacuous (parity gate passes automatically). 2 of 2 P2.1/P2.2 items (@kernel, GPU PTX) fail Python at RUNTIME (not parse) -- different pattern; both compilers must handle the parse. NUANCE CAPTURED: K2-parity-vacuous != Python-deletion-DONE. For bootstrap to be production v1.0 compiler post-K4, it must still IMPLEMENT these shapes (not just no-op-skip). The bootstrap-only-superset pattern shrinks the K2 parity work but doesn't shrink the actual implementation work. Chunk-count estimate stays ~150-235.
