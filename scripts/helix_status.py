...
K_BOOTSTRAP_CHUNKS_DONE = 255      # last bump: K1.F27 -- shipped __tile_matmul(a, b, dst, N) for 2x2 SQUARE matrices, fully unrolled (no loops; N currently ignored). Slot 178; 13-char name. Codegen: ~167 bytes after args (4 dst cells * 38B + 9B setup + 6B end). Math A=[[1,2],[3,4]] x B=[[5,6],[7,8]] = C=[[19,22],[43,50]], sum 134, verified 3/3 runs all 4 cells correct individually. TILE OPS PHASE-0 COMPLETE (5 of 5 real): __tile_zeros, __tile_add, __tile_sub, __tile_mul, __tile_matmul. Category-2 tile-ops row closed. Limitations of initial K1.F27: 2x2 only (general MxN matmul requires 3 nested loops with rel8/rel32 displacement management -- deferred to K1.F27b)
