...
K_BOOTSTRAP_CHUNKS_DONE = 307      # last bump: K2.AH -- speed-up #4 dispatched (parallel K1.F* sweep). Ran `pytest helixc/tests/test_codegen.py -k k1f -n 4 --tb=no` in background to surface ALL currently-broken K1.F* shapes in one parallel sweep instead of one-at-a-time probes. Confirmed pytest-xdist 3.8.0 installed. Sweep takes ~5-8 min with 4 workers (vs ~15 min sequential). Tick 29 will process the results and produce a prioritized failing-shape list. SPEED-UPS SHIPPED THIS SESSION (4 of 5): #1 Python sweep (K2.AE), #2 trap-id helper (K2.AF), #5 5-axis audit protocol (K2.AG), #4 parallel K1.F sweep (K2.AH dispatched). #3 fixture refactor remains deferred (multi-tick).
