...
K_BOOTSTRAP_CHUNKS_DONE = 241      # last bump: K1.F23b -- INVESTIGATION: 6 probes localize the K1.F23 defect. Plain multi-fn works (C/D/E); helper calling print_int works (F); helper calling __arena_len (arena READ) works (G); helper calling __arena_set (indexed arena WRITE) ALSO works (H). The defect is narrower than initially thought: ONLY __arena_push (the cursor-bump write) fails from helper context. The fix scope is small -- the cursor-bump byte sequence (mov ecx,[rax]; cmp ecx,CAP; jb +7; mov [rax+rcx*4+4],edx; inc ecx; mov [rax],ecx; mov eax,edx) misfires in helper context, while __arena_set's similar-but-simpler write succeeds. Pinned via expanded test_bootstrap_kovc_k1f23b_multi_fn_defect_probes_self_host. K_BOOTSTRAP_TILE_OPS_PLAN.md updated
