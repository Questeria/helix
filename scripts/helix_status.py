...
K_BOOTSTRAP_CHUNKS_DONE = 427      # last bump: K3 chunk 2 DONE -- HELIX-NATIVE self-rebuild seed driver works end-to-end with NO Python orchestration and NO external ulimit (test_bootstrap_seed_driver_self_rebuild). A bootstrap-built seed (606KB = full compiler + seed_main) reads the 3 source files (free append-concat via read_file_to_arena), prepends a driver main FIRST (resolve_program_root picks it -> no demo-strip needed), compiles them into a 593KB K-next, then has K-next compile 6*7 and runs the result -> exit 42. Two design corrections from probing: (1) the seed must INCLUDE the compiler source to call lex/parse_top/emit_elf (else unresolved -> SIGILL 132); (2) it must be built by K1/bootstrap NOT Python (the Python reference compiler lacks the run_process/set_exec builtins). The task-16 big-stack stub is what lets the seed self-compile its 1.43MB source without ulimit. Test-only (collect+run: 1 passed). REMAINING trust root = stage-0 hex0/Mes/C (major deferred, user-steered); task 17 (Python-backend stub) to unskip the canonical loop.
