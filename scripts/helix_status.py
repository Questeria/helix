...
K_BOOTSTRAP_CHUNKS_DONE = 295      # last bump: K1.F5d -- substrate-only chunk adding the last_call_ret_struct_idx side-channel (sb+74) that future K1.F5e/g will use to enable chained method-call dispatch. Three pieces: (1) accessor functions last_call_ret_struct_idx() / set_last_call_ret_struct_idx() (parser.hx:174-175, parallel to the last_struct_idx pattern at sb+19). (2) Doc-comment block explaining the slot purpose and the K1.F5d-g closure plan. (3) Init to -1 in install_keywords so first read returns "no struct tracked" instead of arena garbage. No writes or reads yet -- pure substrate. K1.F5c regression intact. K1.F5e will write to it at fn-decl time; K1.F5g will read it from the method-call PRE-CHECK.
