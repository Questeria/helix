...
K_BOOTSTRAP_CHUNKS_DONE = 297      # last bump: K2.AA -- structural finding for the impl-block self-receiver pattern. Probed Python helixc's behavior: `fn read(self) -> i32 { ... }` raises ParseError 'expected COLON (got RPAREN)' at position right after `self`. So `impl P { fn read(self)/fn read(&self) ... }` is BOOTSTRAP-ONLY territory (parallel to macros + tile ops -- bootstrap is a feature-superset of Python). K2 parity harness can never test this shape; the K1.F5h2-i codegen-side work ships as bootstrap-only self-host tests, NOT parity probes. Updated docs/CATEGORY_2_NEXT_PHASE.md P1.2 section with this structural-finding callout. Recorded in runtime/memory/semantic/helix.md as a durable finding so future ticks don't re-probe. No code change -- pure docs + memory.
