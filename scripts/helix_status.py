...
K_BOOTSTRAP_CHUNKS_DONE = 254      # last bump: K1.F26 -- shipped __tile_mul elementwise multiply builtin (slot 177) + retracted K1.F25's mistaken "__arena_get u8-narrowing defect" claim. Codegen mirrors __tile_add scaffold but with imul (opcode 0F AF, 5 bytes) replacing add (opcode 03, 4 bytes); loop body grows from 43 to 44 bytes; jge displacement +40 (was +39), jmp displacement -44 (was -43). Single-cell verified (4*3=12) and multi-cell verified (sum 12+21+32+45=110), all 3/3 runs. Also retracted K1.F25 claim: the "u8 narrowing" was actually KERNEL exit-code-u8 truncation (sys_exit(N) only returns N & 0xFF to parent per POSIX). Bootstrap and __arena_get are both correct -- 1050 in eax becomes exit code 26 because the kernel masks high bits. Tile ops Phase-0: 4 of 5 real (__tile_zeros, __tile_add, __tile_sub, __tile_mul). Remaining: __tile_matmul (2D nested loop with reduction accumulator)
