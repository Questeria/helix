...
**Current continuation pointer**: Stage 35 audit cleanup. Continue from the newest pushed HEAD shown by `git log -1 --oneline`, not from any older fixed hash in this historical handoff. Restart 46 is the latest recorded fix sweep in this file; clean gates remain `0/3`, and live `helixc/tests` collection is 2,437. Run `git status --short` and tail `docs/stage35-progress-2026-05-15.md` for the newest truth.
...
- Heavy gate must be GREEN before declaring clean. Historical snapshot had 670+ tests; Stage 35 restart 46 fix verification collected 2,437 live `helixc/tests` tests, so refresh with `python -m pytest helixc/tests --collect-only -q`.
