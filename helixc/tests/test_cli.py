...
def test_stage59_list_traced_fns_json(tmp_path):
    """Stage 59 follow-on / Tier 3 #11 polish: --list-traced-fns-json
    outputs JSON {"traced_fns": ["fn1", ...]} sorted alphabetically."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "t.hx"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--list-traced-fns-json", str(src)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"traced_fns": ["a", "b"]}
...
def test_stage59_list_uses_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --list-uses-json emits
    {uses: [{path, segments}]} with both joined path and raw segments."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "uj.hx"
...
def test_stage59_agent_methods_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --agent-methods-json
    emits JSON {methods: [{name, params, return_ty}]}."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "amj.hx"
    src.write_text(
        ...
        "    fn evaluate(s: i32, h: i32) -> i32;\n"
        "}\n",
        encoding="utf-8",
    )
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--agent-methods-json", str(src), "Planner"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
...
def test_stage59_fn_loc_text(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-loc prints
    '<path>:<line>:<col>' for a known fn."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "floc.hx"
    src.write_text(
        "fn first() -> i32 { 1 }\n"
        "\n"
        "fn second() -> i32 { 2 }\n",
    ...
    proc1 = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-loc", str(src), "first"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    proc2 = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-loc", str(src), "second"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc1.returncode == 0
    assert proc2.returncode == 0
    # 'first' on line 1, 'second' on line 3.
    assert proc1.stdout.strip().endswith(":1:1")
    assert proc2.stdout.strip().endswith(":3:1")


def test_stage59_fn_loc_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-loc-json emits
    {name, file, line, col, found}."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "flocj.hx"
    src.write_text("fn foo() -> i32 { 1 }\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-loc-json", str(src), "foo"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
...
def test_stage59_fn_loc_missing(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-loc rc=1 for
    unknown fn."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "flocm.hx"
    src.write_text("fn foo() -> i32 { 1 }\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-loc", str(src), "nope"],
...
def test_stage59_ast_node_counts_text(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --ast-node-counts
    prints per-class node counts sorted by frequency."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "anc.hx"
    src.write_text(
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--ast-node-counts", str(src)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    out = proc.stdout
    # FnDecl appears 2 times (2 fns).
    assert "FnDecl: 2" in out
    # Binary appears at least 2 times (one + per fn).
    assert "Binary:" in out


def test_stage59_ast_node_counts_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --ast-node-counts-json
    emits {counts, total_nodes, n_classes}."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "ancj.hx"
    src.write_text(
        "fn foo() -> i32 { 1 + 2 }\n",
        encoding="utf-8",
    )
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--ast-node-counts-json", str(src)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    for key in ("counts", "total_nodes", "n_classes"):
        assert key in result
    # Single fn → FnDecl count is 1.
    assert result["counts"].get("FnDecl") == 1
    # n_classes matches counts size.
    assert result["n_classes"] == len(result["counts"])
    # total_nodes is the sum.
    assert result["total_nodes"] == sum(result["counts"].values())
...
def test_stage59_check_program_hash_from_file_json_match(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish:
    --check-program-hash-from-file-json reads pin from file, emits
    JSON, rc=0 on match."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "cphffj.hx"
    src.write_text("fn foo() -> i32 { 1 }\n", encoding="utf-8")
    ...
    pin_file.write_text(expected_full + "\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--check-program-hash-from-file-json",
         str(src), str(pin_file)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
...
def test_stage59_diff_hash_dump_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --diff-hash-dump-json
    emits structured per-category drift."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    a = tmp_path / "a.hx"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--diff-hash-dump-json", str(a), str(b)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 1
    result = json.loads(proc.stdout)
    assert result["match"] is False
    assert result["fns"]["added"] == ["new_fn"]
    assert result["fns"]["removed"] == ["old_fn"]
    assert result["fns"]["changed_body"] == ["foo"]
    assert result["fns"]["changed_sig"] == []


def test_stage59_diff_hash_dump_json_match(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --diff-hash-dump-json
    emits match=true + all-empty + rc=0 on identical files."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = "fn foo() -> i32 { 1 }\n"
...
def test_stage59_fn_callers_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-callers-json
    emits {target, callers: [...]} with callers alphabetically
    sorted."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "fcaj.hx"
    src.write_text(
        "fn target() -> i32 { 1 }\n"
        "fn b() -> i32 { target() }\n"
        "fn a() -> i32 { target() }\n"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-callers-json", str(src), "target"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"target": "target", "callers": ["a", "b"]}


def test_stage59_fn_callers_json_no_callers(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-callers-json
    emits {target, callers: []} (rc=0) for unknown / unreferenced
    target (matches text-form semantics: empty is valid)."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "fcaj_none.hx"
    src.write_text("fn a() -> i32 { 1 }\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-callers-json", str(src), "nonexistent"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"target": "nonexistent", "callers": []}


def test_stage59_fn_callgraph_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-callgraph-json
    emits {caller, callees: [...]} with callees alphabetically
    sorted."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-callgraph-json", str(src), "c"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"caller": "c", "callees": ["a", "b"]}


def test_stage59_fn_callgraph_json_missing_rc1(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-callgraph-json
    exits 1 when fn not found."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "fcj_miss.hx"
    src.write_text("fn a() -> i32 { 1 }\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-callgraph-json", str(src), "nope"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 1
    assert "not found" in proc.stderr
...
def test_stage59_dump_ast_hashes_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --dump-ast-hashes-json
    emits {fn_name: 64hex} dict (full hashes, not short)."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "dah.hx"
...
def test_stage59_type_alias_target_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --type-alias-target-json
    emits JSON {name, target}."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "tatj.hx"
    src.write_text(
        "type Bytes = i64;\n",
        encoding="utf-8",
    )
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--type-alias-target-json", str(src), "Bytes"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"name": "Bytes", "target": "i64"}
...
def test_stage59_agent_methods(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --agent-methods prints
    method signatures '<name>(<params>) -> <ret>' for one agent."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "am.hx"
    src.write_text(
    ...
    assert lines == [
        "propose(i32) -> i32",
        "evaluate(i32, i32) -> i32",
    ]


def test_stage59_agent_methods_not_found(tmp_path):
    """Stage 59 follow-on: unknown agent → rc=1 + stderr."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "x.hx"
    src.write_text("agent A { fn f() -> i32; }\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--agent-methods", str(src), "phantom"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 1
    assert "not found" in proc.stderr
...
def test_stage59_list_agents_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --list-agents-json emits
    JSON with method counts AND method_names lists."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "aj.hx"
...
def test_stage59_enum_variants_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --enum-variants-json
    emits JSON {variants: [{name, payload_tys}]}."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "evj.hx"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--enum-variants-json", str(src), "Shape"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"variants": [
        {"name": "Circle", "payload_tys": ["f32"]},
        {"name": "Square", "payload_tys": ["i32", "i32"]},
        {"name": "Triangle", "payload_tys": []},
    ]}
...
def test_stage59_list_fn_attrs_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --list-fn-attrs-json
    outputs valid JSON with per-fn attribute list (sorted)."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "a.hx"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--list-fn-attrs-json", str(src)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"a": ["pure"], "b": ["trace"], "c": []}


def test_stage59_list_fns_by_attr_json(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --list-fns-by-attr-json
    emits {attr, fns} JSON sorted alphabetically."""
    import json
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "a.hx"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--list-fns-by-attr-json", str(src), "pure"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    result = json.loads(proc.stdout)
    assert result == {"attr": "pure", "fns": ["both", "pure_add"]}
...
def test_stage59_fn_callgraph_dot(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-callgraph-dot
    emits Graphviz digraph format with sorted edges."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "dot.hx"
    src.write_text(
...
def test_stage59_fn_call_path_chain(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-call-path emits
    shortest BFS path from A to B as 'A -> B -> ... -> Z' line."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "p.hx"
    src.write_text(
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-call-path", str(src), "a", "z"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    assert proc.stdout.strip() == "a -> b -> c -> z"


def test_stage59_fn_call_path_no_path(tmp_path):
    """Stage 59 follow-on: no path from B to A in a forward-only chain
    → 'no path' + rc=0 (not an error)."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "np.hx"
    src.write_text(
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-call-path", str(src), "b", "a"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    assert proc.stdout.strip() == "no path"


def test_stage59_fn_call_path_self(tmp_path):
    """Stage 59 follow-on: from A to A returns just 'A'."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "self.hx"
    src.write_text("fn solo() -> i32 { 0 }\n", encoding="utf-8")
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-call-path", str(src), "solo", "solo"],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    assert proc.stdout.strip() == "solo"


def test_stage59_fn_isolated(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-isolated lists
    orphan fns (no callers AND no callees in local graph)."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "iso.hx"
    src.write_text(
    ...
    assert proc.returncode == 0
    lines = [l for l in proc.stdout.splitlines() if l]
    # util has a caller → not isolated; caller has a callee → not
    # isolated; orphan_a/b have neither → isolated, sorted.
    assert lines == ["orphan_a", "orphan_b"]


def test_stage59_fn_topo_sort_leaves_first(tmp_path):
    """Stage 59 follow-on / Tier 4 #13 polish: --fn-topo-sort emits
    fns in leaves-first order — if A calls B, B appears before A."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "topo.hx"
    src.write_text(
        "fn z() -> i32 { 42 }\n"
        "fn c() -> i32 { z() }\n"
        "fn b() -> i32 { c() }\n"
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "helixc.frontend.autodiff_cli",
         "--fn-topo-sort", str(src)],
        cwd=proj_root, capture_output=True, text=True, timeout=30,
    )
    assert proc.returncode == 0
    lines = [l for l in proc.stdout.splitlines() if l]
    # z leaf first, then c (calls z), then b (calls c), then a (calls b)
    assert lines == ["z", "c", "b", "a"]


def test_stage59_fn_topo_sort_handles_cycles(tmp_path):
    """Stage 59 follow-on: mutual recursion (SCC of size >= 2) is
    handled — both members appear (in an adjacent block); no infinite
    loop or crash."""
    proj_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    src = tmp_path / "cyc.hx"
    ...
    assert proc.returncode == 0
    lines = [l for l in proc.stdout.splitlines() if l]
    # All 3 fns appear; util (leaf) must come before a,b (which call it).
    assert set(lines) == {"util", "a", "b"}
    assert lines.index("util") < lines.index("a")
    assert lines.index("util") < lines.index("b")
