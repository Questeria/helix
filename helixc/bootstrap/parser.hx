...
fn parse_primary(tok_base: i32, sb: i32) -> i32 {
    ...
    if t == 43 {
        ...
        if is_macro_call == 1 {
            ...
            // K1.F22f (2026-05-27): eprint!(STR) detection. 6-char
            // IDENT "eprint" = bytes 101 112 114 105 110 116. Stderr
            // no-newline variant; routes through the K1.F22f eprint_str
            // builtin (slot 173, 10 chars). id_len==6 distinguishes
            // from print (5), println (7), eprintln (8), eprint_str (10).
            let is_eprint_name_macro = if id_len == 6 {
                let pb0e = __arena_get(id_start);
                let pb1e = __arena_get(id_start + 1);
                let pb2e = __arena_get(id_start + 2);
                let pb3e = __arena_get(id_start + 3);
                let pb4e = __arena_get(id_start + 4);
                let pb5e = __arena_get(id_start + 5);
                if pb0e == 101 { if pb1e == 112 { if pb2e == 114 {
                    if pb3e == 105 { if pb4e == 110 { if pb5e == 116 { 1 }
            ...
            // K1.F22j (2026-05-27): assert_eq!(IDENT, IDENT) detection.
            // 9-char IDENT "assert_eq" = bytes 97 115 115 101 114 116
            // 95 101 113. Two-arg comparison macro: synthesizes
            // AST_IF(cond=AST_EQ(AST_VAR(a), AST_VAR(b)), ...). Builds
            // on K1.F22i's AST_IF substrate and adds AST_EQ (tag 20).
            // SCOPE: single-IDENT operands only -- compound forms like
            // `assert_eq!(f(a), b)` fall through to K1.CB no-op-skip.
            let is_assert_eq_name_macro = if id_len == 9 {
                let qb0 = __arena_get(id_start);
                let qb1 = __arena_get(id_start + 1);
                let qb2 = __arena_get(id_start + 2);
                let qb3 = __arena_get(id_start + 3);
                let qb4 = __arena_get(id_start + 4);
                let qb5 = __arena_get(id_start + 5);
                let qb6 = __arena_get(id_start + 6);
                let qb7 = __arena_get(id_start + 7);
                let qb8 = __arena_get(id_start + 8);
                if qb0 == 97 { if qb1 == 115 { if qb2 == 115 {
                    if qb3 == 101 { if qb4 == 114 { if qb5 == 116 {
                    if qb6 == 95 { if qb7 == 101 { if qb8 == 113 { 1 }
            ...
            // K1.F22f (2026-05-27): shape guard for eprint!(STR_LIT).
            ...
            //   mac_t2 = TK_LPAREN (3)
            //   mac_t3 = TK_IDENT  (2)  <- the left operand
            //   mac_t4 = TK_COMMA  (13)
            ...
            // K1.F43 (2026-05-27): mac_t7 added for 8-token shapes (double-
            // char compare ops like `==`, `!=`, `<=`, `>=` which lex as two
            // separate TK tokens; see K1.F41 commit for the structural finding).
            let mac_t7 = tok_tag(tok_base, k + 7);
            ...
            // The lexer emits single `<` as TK_LT (16); double-char `<=` is
            // lex'd as TK_LT + TK_EQ (two tokens), so this detector ONLY
            // matches the strict `<` form. The `<=` 8-token shape is a
            // future chunk.
            let is_assert_ident_lt_int_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 16 {
                    if mac_t5 == 1 { if mac_t6 == 4 {
                        let alti_a_opnd_s = tok_p2(tok_base, k + 3);
                        let alti_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(alti_a_opnd_s, alti_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F42 (2026-05-27): assert!(IDENT > INT_LIT). Sibling of K1.F41
            // for the strict-greater form. Same 7-token shape, mac_t4=TK_GT(17).
            let is_assert_ident_gt_int_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 17 {
                    if mac_t5 == 1 { if mac_t6 == 4 {
                        let agti_a_opnd_s = tok_p2(tok_base, k + 3);
                        let agti_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(agti_a_opnd_s, agti_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F43 (2026-05-27): assert!(IDENT == INT_LIT). Double-char `==`
            // lexes as TWO tokens (TK_EQ TK_EQ, both tag 15), so this is an
            // 8-token shape (vs F41/F42 7-token strict-inequality forms).
            // mac_t2=LPAREN(3), mac_t3=IDENT(2), mac_t4=TK_EQ(15), mac_t5=
            // TK_EQ(15), mac_t6=TK_INT(1), mac_t7=TK_RPAREN(4).
            let is_assert_ident_eq_int_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 15 {
                    if mac_t5 == 15 { if mac_t6 == 1 { if mac_t7 == 4 {
                        let aeqi_a_opnd_s = tok_p2(tok_base, k + 3);
                        let aeqi_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(aeqi_a_opnd_s, aeqi_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F44 (2026-05-27): assert!(IDENT != INT_LIT). Double-char `!=`
            // lexes as TK_BANG (18) + TK_EQ (15). mac_t4=18, mac_t5=15.
            let is_assert_ident_ne_int_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 18 {
                    if mac_t5 == 15 { if mac_t6 == 1 { if mac_t7 == 4 {
                        let anei_a_opnd_s = tok_p2(tok_base, k + 3);
                        let anei_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(anei_a_opnd_s, anei_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F45 (2026-05-27): assert!(IDENT <= INT_LIT). Double-char `<=`
            // lexes as TK_LT (16) + TK_EQ (15). mac_t4=16, mac_t5=15. Uses
            // AST_LE (tag 22). Closes the assert! comparison family along
            // with K1.F46 (`>=`).
            let is_assert_ident_le_int_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 16 {
                    if mac_t5 == 15 { if mac_t6 == 1 { if mac_t7 == 4 {
                        let alei_a_opnd_s = tok_p2(tok_base, k + 3);
                        let alei_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(alei_a_opnd_s, alei_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F46 (2026-05-27): assert!(IDENT >= INT_LIT). Double-char `>=`
            // lexes as TK_GT (17) + TK_EQ (15). Uses AST_GE (tag 23).
            let is_assert_ident_ge_int_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 17 {
                    if mac_t5 == 15 { if mac_t6 == 1 { if mac_t7 == 4 {
                        let agei_a_opnd_s = tok_p2(tok_base, k + 3);
                        let agei_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(agei_a_opnd_s, agei_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            ...
            // K1.F49 (2026-05-27): assert!(IDENT == IDENT). 8-token shape.
            // mac_t4=TK_EQ(15), mac_t5=TK_EQ(15), mac_t6=TK_IDENT(2),
            // mac_t7=TK_RPAREN(4). Disjoint from K1.F43 (mac_t6=TK_INT)
            // via mac_t6 tag. Both operands K3.S-reject-checked.
            let is_assert_ident_eq_ident_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 15 {
                    if mac_t5 == 15 { if mac_t6 == 2 { if mac_t7 == 4 {
                        let aeqd_a_s = tok_p2(tok_base, k + 3);
                        let aeqd_a_l = tok_p3(tok_base, k + 3);
                        let aeqd_b_s = tok_p2(tok_base, k + 6);
                        let aeqd_b_l = tok_p3(tok_base, k + 6);
                        if is_any_reserved_kw_ident(aeqd_a_s, aeqd_a_l) == 1 { 0 }
                        else { if is_any_reserved_kw_ident(aeqd_b_s, aeqd_b_l) == 1 { 0 }
                        else { 1 } }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F50 (2026-05-27): assert!(IDENT != IDENT). Sibling of K1.F49.
            // mac_t4=TK_BANG(18), mac_t5=TK_EQ(15). Disjoint from K1.F44 via
            // mac_t6 tag (IDENT vs INT).
            let is_assert_ident_ne_ident_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 18 {
                    if mac_t5 == 15 { if mac_t6 == 2 { if mac_t7 == 4 {
                        let aned_a_s = tok_p2(tok_base, k + 3);
                        let aned_a_l = tok_p3(tok_base, k + 3);
                        let aned_b_s = tok_p2(tok_base, k + 6);
                        let aned_b_l = tok_p3(tok_base, k + 6);
                        if is_any_reserved_kw_ident(aned_a_s, aned_a_l) == 1 { 0 }
                        else { if is_any_reserved_kw_ident(aned_b_s, aned_b_l) == 1 { 0 }
                        else { 1 } }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F51 (2026-05-27): assert!(IDENT <= IDENT). 8-token shape.
            // mac_t4=TK_LT(16), mac_t5=TK_EQ(15), mac_t6=TK_IDENT(2),
            // mac_t7=TK_RPAREN(4). AST_LE (22). Disjoint from K1.F45 via
            // mac_t6 (TK_IDENT vs TK_INT).
            let is_assert_ident_le_ident_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 16 {
                    if mac_t5 == 15 { if mac_t6 == 2 { if mac_t7 == 4 {
                        let aled_a_s = tok_p2(tok_base, k + 3);
                        let aled_a_l = tok_p3(tok_base, k + 3);
                        let aled_b_s = tok_p2(tok_base, k + 6);
                        let aled_b_l = tok_p3(tok_base, k + 6);
                        if is_any_reserved_kw_ident(aled_a_s, aled_a_l) == 1 { 0 }
                        else { if is_any_reserved_kw_ident(aled_b_s, aled_b_l) == 1 { 0 }
                        else { 1 } }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F52 (2026-05-27): assert!(IDENT >= IDENT). Sibling of K1.F51.
            // mac_t4=TK_GT(17). AST_GE (23). Closes the assert! comparison
            // family across both operand shapes for all 6 ops.
            let is_assert_ident_ge_ident_form = if is_assert_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 17 {
                    if mac_t5 == 15 { if mac_t6 == 2 { if mac_t7 == 4 {
                        let aged_a_s = tok_p2(tok_base, k + 3);
                        let aged_a_l = tok_p3(tok_base, k + 3);
                        let aged_b_s = tok_p2(tok_base, k + 6);
                        let aged_b_l = tok_p3(tok_base, k + 6);
                        if is_any_reserved_kw_ident(aged_a_s, aged_a_l) == 1 { 0 }
                        else { if is_any_reserved_kw_ident(aged_b_s, aged_b_l) == 1 { 0 }
                        else { 1 } }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            let is_assert_eq_form = if is_assert_eq_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 13 {
                    if mac_t5 == 2 { if mac_t6 == 4 {
                        ...
                        if is_any_reserved_kw_ident(aeq_a_opnd_s, aeq_a_opnd_l) == 1 { 0 }
                        else { if is_any_reserved_kw_ident(aeq_b_opnd_s, aeq_b_opnd_l) == 1 { 0 }
                        else { 1 } }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            // K1.F31 (2026-05-27): assert_eq!(IDENT, INT_LIT) shape -- 7 tokens.
            // mac_t3 = TK_IDENT (2), mac_t4 = TK_COMMA (13), mac_t5 = TK_INT (1),
            // mac_t6 = TK_RPAREN (4). Synthesis: AST_IF(AST_EQ(AST_VAR(a),
            // AST_INT(b)), then=0, else=panic). Same K3.S reject on the IDENT
            // operand to avoid the AST_VAR-on-keyword silent miscompile.
            let is_assert_eq_ident_int_form = if is_assert_eq_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 13 {
                    if mac_t5 == 1 { if mac_t6 == 4 {
                        let aeqii_a_opnd_s = tok_p2(tok_base, k + 3);
                        let aeqii_a_opnd_l = tok_p3(tok_base, k + 3);
                        if is_any_reserved_kw_ident(aeqii_a_opnd_s, aeqii_a_opnd_l) == 1 { 0 }
                        else { 1 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            ...
            // K1.F35 (2026-05-27): assert_eq!(IDENT, BOOL_LIT). Real Rust:
            // `assert_eq!(flag, true)` or `assert_eq!(flag, false)`. Pre-F35
            // this fell through to K1.CB no-op-skip: K1.F22j rejected it (b is
            // reserved kw, K3.Q-style guard) and K1.F22j2 rejected it (a is
            // non-bool-lit). 7-token shape; mac_t3=IDENT(2), mac_t5=IDENT(2).
            // Detection: a NOT bool-lit AND b IS bool-lit. The IDENT-a side
            // also passes the K3.S reserved-kw reject so AST_VAR(a) is safe.
            let is_assert_eq_ident_bool_form = if is_assert_eq_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 13 {
                    if mac_t5 == 2 { if mac_t6 == 4 {
                        let aeqib_a_s = tok_p2(tok_base, k + 3);
                        let aeqib_a_l = tok_p3(tok_base, k + 3);
                        let aeqib_b_s = tok_p2(tok_base, k + 5);
                        let aeqib_b_l = tok_p3(tok_base, k + 5);
                        let aeqib_b_is_bool = if is_kw_true_ident(aeqib_b_s, aeqib_b_l) == 1 { 1 }
                            else { if is_kw_false_ident(aeqib_b_s, aeqib_b_l) == 1 { 1 } else { 0 } };
                        if aeqib_b_is_bool == 1 {
                            if is_any_reserved_kw_ident(aeqib_a_s, aeqib_a_l) == 1 { 0 }
                            else { 1 }
                        } else { 0 }
                    } else { 0 } } else { 0 } } else { 0 } } else { 0 } } else { 0 }
            } else { 0 };
            ...
            // K1.F36 (2026-05-27): assert_ne!(IDENT, BOOL_LIT). Sibling of
            // K1.F35; AST_NE cond + "!=" message. Pattern identical to F35.
            let is_assert_ne_ident_bool_form = if is_assert_ne_name_macro == 1 {
                if mac_t2 == 3 { if mac_t3 == 2 { if mac_t4 == 13 {
                    if mac_t5 == 2 { if mac_t6 == 4 {
                        let aneib_a_s = tok_p2(tok_base, k + 3);
                        let aneib_a_l = tok_p3(tok_base, k + 3);
                        let aneib_b_s = tok_p2(tok_base, k + 5);
                        let aneib_b_l = tok_p3(tok_base, k + 5);
                        let aneib_b_is_bool = if is_kw_true_ident(aneib_b_s, aneib_b_l) == 1 { 1 }
            ...
            if is_panic_str_form == 1 {
            ...
            } else { if is_eprint_str_form == 1 {
                // K1.F22f: synthesize AST_CALL(eprint_str, str_arg).
                // No-newline stderr variant -- routes through the new
                // K1.F22f eprint_str builtin (bn_state slot 173, 10
                // chars "eprint_str") whose codegen is K1.AK print_str
                // with `mov edi, 2` instead of `mov edi, 1`. Same
                // 5-token-consumption shape as the print!/println!/
                // eprintln! arms.
                let ep_str_tok = k + 3;
                let ep_str_body_s = tok_p2(tok_base, ep_str_tok);
                let ep_str_body_l = tok_p3(tok_base, ep_str_tok);
                cur_advance(sb);
                cur_advance(sb);
                cur_advance(sb);
                cur_advance(sb);
                cur_advance(sb);
                let ep_str_ast = mk_node(25, ep_str_body_s, ep_str_body_l, 0);
                let ep_args_head = mk_node(17, ep_str_ast, 0, 0);
                // Push "eprint_str" name bytes (10 chars: 101 112 114
                // 105 110 116 95 115 116 114).
                let ep_name_s = __arena_push(101);
                __arena_push(112); __arena_push(114); __arena_push(105);
                __arena_push(110); __arena_push(116); __arena_push(95);
                __arena_push(115); __arena_push(116); __arena_push(114);
            ...
            } else { if is_assert_ident_lt_int_form == 1 {
                // K1.F41 (2026-05-27): assert!(IDENT < INT_LIT).
                // Synthesizes AST_IF(AST_LT(AST_VAR(a), AST_INT(b_val)),
                //                    then=AST_INT(0),
                //                    else=AST_CALL(panic, "assertion failed: <")).
                // AST_LT tag = 6. Panic message is "assertion failed: <" (19
                // chars: 16 base + ':' + ' ' + '<').
                let alti_a_s = tok_p2(tok_base, k + 3);
                let alti_a_l = tok_p3(tok_base, k + 3);
                let alti_b_val = tok_p1(tok_base, k + 5);
                cur_advance(sb);           // IDENT (assert)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                ...
                cur_advance(sb);           // )
                let alti_var_a = mk_node(1, alti_a_s, alti_a_l, 0);
                let alti_int_b = mk_node(0, alti_b_val, 0, 0);
                let alti_cond = mk_node(6, alti_var_a, alti_int_b, 0);   // AST_LT
                let alti_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                let alti_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(32);                                         // ' '
                __arena_push(60);                                         // '<'
                let alti_str_ast = mk_node(25, alti_msg_s, 19, 0);
                let alti_args_head = mk_node(17, alti_str_ast, 0, 0);
                let alti_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let alti_else = mk_node(16, alti_panic_name_s, 5, alti_args_head);
                mk_node(7, alti_cond, alti_then, alti_else)
            } else { if is_assert_ident_gt_int_form == 1 {
                // K1.F42 (2026-05-27): assert!(IDENT > INT_LIT). Sibling of
                // K1.F41 with AST_GT (tag 19) and '>' in the panic message.
                let agti_a_s = tok_p2(tok_base, k + 3);
                let agti_a_l = tok_p3(tok_base, k + 3);
                let agti_b_val = tok_p1(tok_base, k + 5);
                cur_advance(sb);           // IDENT (assert)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                ...
                cur_advance(sb);           // INT (b)
                cur_advance(sb);           // )
                let agti_var_a = mk_node(1, agti_a_s, agti_a_l, 0);
                let agti_int_b = mk_node(0, agti_b_val, 0, 0);
                let agti_cond = mk_node(19, agti_var_a, agti_int_b, 0);  // AST_GT
                let agti_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                let agti_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(32);                                         // ' '
                __arena_push(62);                                         // '>'
                let agti_str_ast = mk_node(25, agti_msg_s, 19, 0);
                let agti_args_head = mk_node(17, agti_str_ast, 0, 0);
                let agti_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let agti_else = mk_node(16, agti_panic_name_s, 5, agti_args_head);
                mk_node(7, agti_cond, agti_then, agti_else)
            } else { if is_assert_ident_eq_int_form == 1 {
                // K1.F43 (2026-05-27): assert!(IDENT == INT_LIT). 8-token
                // shape (the `==` lexes as two TK_EQ tokens). The INT_LIT
                // is at k+6, not k+5 like the 7-token forms. Cur_advance
                // fires 8 times to consume the extra `=`.
                let aeqi_a_s = tok_p2(tok_base, k + 3);
                let aeqi_a_l = tok_p3(tok_base, k + 3);
                let aeqi_b_val = tok_p1(tok_base, k + 6);
                cur_advance(sb);           // IDENT (assert)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                cur_advance(sb);           // = (first half of ==)
                cur_advance(sb);           // = (second half of ==)
                cur_advance(sb);           // INT (b)
                cur_advance(sb);           // )
                let aeqi_var_a = mk_node(1, aeqi_a_s, aeqi_a_l, 0);
                let aeqi_int_b = mk_node(0, aeqi_b_val, 0, 0);
                let aeqi_cond = mk_node(20, aeqi_var_a, aeqi_int_b, 0);  // AST_EQ
                let aeqi_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                // Reuse K1.F22j 20-byte "assertion failed: ==" message.
                let aeqi_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                let aeqi_str_ast = mk_node(25, aeqi_msg_s, 20, 0);
                let aeqi_args_head = mk_node(17, aeqi_str_ast, 0, 0);
                let aeqi_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let aeqi_else = mk_node(16, aeqi_panic_name_s, 5, aeqi_args_head);
                mk_node(7, aeqi_cond, aeqi_then, aeqi_else)
            } else { if is_assert_ident_ne_int_form == 1 {
                // K1.F44 (2026-05-27): assert!(IDENT != INT_LIT). 8-token
                // shape (`!=` lexes as TK_BANG TK_EQ). Sibling of K1.F43
                // with AST_NE (tag 21) and "!=" message bytes.
                let anei_a_s = tok_p2(tok_base, k + 3);
                let anei_a_l = tok_p3(tok_base, k + 3);
                let anei_b_val = tok_p1(tok_base, k + 6);
                cur_advance(sb);           // IDENT (assert)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                ...
                cur_advance(sb);           // INT (b)
                cur_advance(sb);           // )
                let anei_var_a = mk_node(1, anei_a_s, anei_a_l, 0);
                let anei_int_b = mk_node(0, anei_b_val, 0, 0);
                let anei_cond = mk_node(21, anei_var_a, anei_int_b, 0);  // AST_NE
                let anei_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                // Reuse K1.F22k 20-byte "assertion failed: !=" message.
                let anei_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                let anei_str_ast = mk_node(25, anei_msg_s, 20, 0);
                let anei_args_head = mk_node(17, anei_str_ast, 0, 0);
                let anei_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let anei_else = mk_node(16, anei_panic_name_s, 5, anei_args_head);
                mk_node(7, anei_cond, anei_then, anei_else)
            } else { if is_assert_ident_le_int_form == 1 {
                // K1.F45 (2026-05-27): assert!(IDENT <= INT_LIT). 8-token
                // shape (`<=` = TK_LT TK_EQ). AST_LE tag = 22. Panic message
                // "assertion failed: <=" = 20 bytes (same as "==").
                let alei_a_s = tok_p2(tok_base, k + 3);
                let alei_a_l = tok_p3(tok_base, k + 3);
                let alei_b_val = tok_p1(tok_base, k + 6);
                cur_advance(sb);           // IDENT (assert)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                ...
                cur_advance(sb);           // INT (b)
                cur_advance(sb);           // )
                let alei_var_a = mk_node(1, alei_a_s, alei_a_l, 0);
                let alei_int_b = mk_node(0, alei_b_val, 0, 0);
                let alei_cond = mk_node(22, alei_var_a, alei_int_b, 0);  // AST_LE
                let alei_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                let alei_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(58);                                         // ':'
                __arena_push(32);                                         // ' '
                __arena_push(60); __arena_push(61);                       // '<' '='
                let alei_str_ast = mk_node(25, alei_msg_s, 20, 0);
                let alei_args_head = mk_node(17, alei_str_ast, 0, 0);
                let alei_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let alei_else = mk_node(16, alei_panic_name_s, 5, alei_args_head);
                mk_node(7, alei_cond, alei_then, alei_else)
            } else { if is_assert_ident_ge_int_form == 1 {
                // K1.F46 (2026-05-27): assert!(IDENT >= INT_LIT). Sibling of
                // K1.F45 with AST_GE (tag 23) and ">=" message. Closes the
                // 6-op assert! comparison family.
                let agei_a_s = tok_p2(tok_base, k + 3);
                let agei_a_l = tok_p3(tok_base, k + 3);
                let agei_b_val = tok_p1(tok_base, k + 6);
                cur_advance(sb);           // IDENT (assert)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                ...
                cur_advance(sb);           // INT (b)
                cur_advance(sb);           // )
                let agei_var_a = mk_node(1, agei_a_s, agei_a_l, 0);
                let agei_int_b = mk_node(0, agei_b_val, 0, 0);
                let agei_cond = mk_node(23, agei_var_a, agei_int_b, 0);  // AST_GE
                let agei_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                let agei_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(58);                                         // ':'
                __arena_push(32);                                         // ' '
                __arena_push(62); __arena_push(61);                       // '>' '='
                let agei_str_ast = mk_node(25, agei_msg_s, 20, 0);
                let agei_args_head = mk_node(17, agei_str_ast, 0, 0);
                let agei_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
            ...
            } else { if is_assert_ident_eq_ident_form == 1 {
                // K1.F49 (2026-05-27): assert!(IDENT == IDENT). 8-token shape.
                // AST_EQ (20) on both AST_VAR operands. b is at k+6 (vs k+5
                // for 7-token shape). 20-byte "assertion failed: ==" message.
                let aeqd_a_s = tok_p2(tok_base, k + 3);
                let aeqd_a_l = tok_p3(tok_base, k + 3);
                let aeqd_b_s = tok_p2(tok_base, k + 6);
                let aeqd_b_l = tok_p3(tok_base, k + 6);
                ...
                cur_advance(sb);           // IDENT (b)
                cur_advance(sb);           // )
                let aeqd_var_a = mk_node(1, aeqd_a_s, aeqd_a_l, 0);
                let aeqd_var_b = mk_node(1, aeqd_b_s, aeqd_b_l, 0);
                let aeqd_cond = mk_node(20, aeqd_var_a, aeqd_var_b, 0);  // AST_EQ
                let aeqd_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                let aeqd_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(32);                                         // ' '
                __arena_push(61); __arena_push(61);                       // '=' '='
                let aeqd_str_ast = mk_node(25, aeqd_msg_s, 20, 0);
                let aeqd_args_head = mk_node(17, aeqd_str_ast, 0, 0);
                let aeqd_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let aeqd_else = mk_node(16, aeqd_panic_name_s, 5, aeqd_args_head);
                mk_node(7, aeqd_cond, aeqd_then, aeqd_else)
            } else { if is_assert_ident_ne_ident_form == 1 {
                // K1.F50 (2026-05-27): assert!(IDENT != IDENT). AST_NE (21).
                let aned_a_s = tok_p2(tok_base, k + 3);
                let aned_a_l = tok_p3(tok_base, k + 3);
                let aned_b_s = tok_p2(tok_base, k + 6);
                let aned_b_l = tok_p3(tok_base, k + 6);
                ...
                cur_advance(sb);           // IDENT (b)
                cur_advance(sb);           // )
                let aned_var_a = mk_node(1, aned_a_s, aned_a_l, 0);
                let aned_var_b = mk_node(1, aned_b_s, aned_b_l, 0);
                let aned_cond = mk_node(21, aned_var_a, aned_var_b, 0);  // AST_NE
                let aned_then = mk_node(0, 0, 0, 0);
                let aned_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(32);                                         // ' '
                __arena_push(33); __arena_push(61);                       // '!' '='
                let aned_str_ast = mk_node(25, aned_msg_s, 20, 0);
                let aned_args_head = mk_node(17, aned_str_ast, 0, 0);
                let aned_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let aned_else = mk_node(16, aned_panic_name_s, 5, aned_args_head);
                mk_node(7, aned_cond, aned_then, aned_else)
            } else { if is_assert_ident_le_ident_form == 1 {
                // K1.F51 (2026-05-27): assert!(IDENT <= IDENT). AST_LE (22).
                // 8 cur_advance; b at k+6. 20-byte "<=" message.
                let aled_a_s = tok_p2(tok_base, k + 3);
                let aled_a_l = tok_p3(tok_base, k + 3);
                let aled_b_s = tok_p2(tok_base, k + 6);
                let aled_b_l = tok_p3(tok_base, k + 6);
                ...
                cur_advance(sb);           // IDENT (b)
                cur_advance(sb);           // )
                let aled_var_a = mk_node(1, aled_a_s, aled_a_l, 0);
                let aled_var_b = mk_node(1, aled_b_s, aled_b_l, 0);
                let aled_cond = mk_node(22, aled_var_a, aled_var_b, 0);  // AST_LE
                let aled_then = mk_node(0, 0, 0, 0);                      // AST_INT(0)
                let aled_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(32);                                         // ' '
                __arena_push(60); __arena_push(61);                       // '<' '='
                let aled_str_ast = mk_node(25, aled_msg_s, 20, 0);
                let aled_args_head = mk_node(17, aled_str_ast, 0, 0);
                let aled_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let aled_else = mk_node(16, aled_panic_name_s, 5, aled_args_head);
                mk_node(7, aled_cond, aled_then, aled_else)
            } else { if is_assert_ident_ge_ident_form == 1 {
                // K1.F52 (2026-05-27): assert!(IDENT >= IDENT). AST_GE (23).
                // CLOSES the assert! comparison family for both shapes x
                // 6 ops = 12 chunks F41-F52.
                let aged_a_s = tok_p2(tok_base, k + 3);
                let aged_a_l = tok_p3(tok_base, k + 3);
                let aged_b_s = tok_p2(tok_base, k + 6);
                let aged_b_l = tok_p3(tok_base, k + 6);
                ...
                cur_advance(sb);           // IDENT (b)
                cur_advance(sb);           // )
                let aged_var_a = mk_node(1, aged_a_s, aged_a_l, 0);
                let aged_var_b = mk_node(1, aged_b_s, aged_b_l, 0);
                let aged_cond = mk_node(23, aged_var_a, aged_var_b, 0);  // AST_GE
                let aged_then = mk_node(0, 0, 0, 0);
                let aged_msg_s = __arena_push(97);                        // 'a'
                __arena_push(115); __arena_push(115);                     // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);  // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);  // 'i' 'o' 'n'
                __arena_push(32);                                         // ' '
                ...
                __arena_push(32);                                         // ' '
                __arena_push(62); __arena_push(61);                       // '>' '='
                let aged_str_ast = mk_node(25, aged_msg_s, 20, 0);
                let aged_args_head = mk_node(17, aged_str_ast, 0, 0);
                let aged_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let aged_else = mk_node(16, aged_panic_name_s, 5, aged_args_head);
                mk_node(7, aged_cond, aged_then, aged_else)
                ...
                // "assertion failed: ==")). Builds on K1.F22i's AST_IF
                // synthesis and adds AST_EQ (tag 20).
                let aeq_a_tok = k + 3;
                let aeq_a_s = tok_p2(tok_base, aeq_a_tok);
                let aeq_a_l = tok_p3(tok_base, aeq_a_tok);
                let aeq_b_tok = k + 5;
                let aeq_b_s = tok_p2(tok_base, aeq_b_tok);
                let aeq_b_l = tok_p3(tok_base, aeq_b_tok);
                cur_advance(sb);           // IDENT (assert_eq)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                cur_advance(sb);           // ,
                cur_advance(sb);           // IDENT (b)
                cur_advance(sb);           // )  (7 advances total)
                let aeq_var_a = mk_node(1, aeq_a_s, aeq_a_l, 0);
                let aeq_var_b = mk_node(1, aeq_b_s, aeq_b_l, 0);
                let aeq_cond = mk_node(20, aeq_var_a, aeq_var_b, 0);  // AST_EQ
                let aeq_then = mk_node(0, 0, 0, 0);                    // AST_INT(0)
                // Push "assertion failed: ==" message bytes (20 chars):
                //   a s s e r t i o n SP f a i l e d : SP = =
                let aeq_msg_s = __arena_push(97);                          // 'a'
                __arena_push(115); __arena_push(115);                      // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);   // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);   // 'i' 'o' 'n'
                __arena_push(32);                                          // ' '
                ...
                __arena_push(32);                                          // ' '
                __arena_push(61); __arena_push(61);                        // '=' '='
                let aeq_str_ast = mk_node(25, aeq_msg_s, 20, 0);
                let aeq_args_head = mk_node(17, aeq_str_ast, 0, 0);
                let aeq_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
                let aeq_else = mk_node(16, aeq_panic_name_s, 5, aeq_args_head);
                mk_node(7, aeq_cond, aeq_then, aeq_else)
            } else { if is_assert_eq_ident_int_form == 1 {
                // K1.F31 (2026-05-27): assert_eq!(IDENT, INT_LIT) form.
                // Synthesizes AST_IF(cond=AST_EQ(AST_VAR(a), AST_INT(b_val)),
                //                    then=AST_INT(0),
                //                    else=AST_CALL(panic, "assertion failed: ==")).
                // Sibling of K1.F22j; AST_INT on the right side instead of
                // AST_VAR. 7-token consumption (assert_eq, !, (, IDENT, comma,
                // INT, )). K3.S reject applies to the IDENT operand.
                let aeqii_a_s = tok_p2(tok_base, k + 3);
                let aeqii_a_l = tok_p3(tok_base, k + 3);
                ...
                cur_advance(sb);           // INT (b)
                cur_advance(sb);           // )
                let aeqii_var_a = mk_node(1, aeqii_a_s, aeqii_a_l, 0);
                let aeqii_int_b = mk_node(0, aeqii_b_val, 0, 0);
                let aeqii_cond = mk_node(20, aeqii_var_a, aeqii_int_b, 0);  // AST_EQ
                let aeqii_then = mk_node(0, 0, 0, 0);                        // AST_INT(0)
                // Push "assertion failed: ==" message bytes (20 chars).
                let aeqii_msg_s = __arena_push(97);                          // 'a'
                __arena_push(115); __arena_push(115);                        // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);     // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);     // 'i' 'o' 'n'
                __arena_push(32);                                            // ' '
                ...
                __arena_push(32);                                            // ' '
                __arena_push(61); __arena_push(61);                          // '=' '='
                let aeqii_str_ast = mk_node(25, aeqii_msg_s, 20, 0);
                let aeqii_args_head = mk_node(17, aeqii_str_ast, 0, 0);
                let aeqii_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
            ...
            } else { if is_assert_eq_ident_bool_form == 1 {
                // K1.F35 (2026-05-27): assert_eq!(IDENT, BOOL_LIT). Synthesizes
                // AST_IF(AST_EQ(AST_VAR(a), AST_INT(b_val)), 0, panic). Bool
                // value is constant-folded: true -> 1, false -> 0.
                let aeqib_a_s = tok_p2(tok_base, k + 3);
                let aeqib_a_l = tok_p3(tok_base, k + 3);
                let aeqib_b_s = tok_p2(tok_base, k + 5);
                let aeqib_b_l = tok_p3(tok_base, k + 5);
                let aeqib_b_val = if is_kw_true_ident(aeqib_b_s, aeqib_b_l) == 1 { 1 } else { 0 };
                cur_advance(sb);           // IDENT (assert_eq)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                ...
                cur_advance(sb);           // IDENT (true/false)
                cur_advance(sb);           // )
                let aeqib_var_a = mk_node(1, aeqib_a_s, aeqib_a_l, 0);
                let aeqib_int_b = mk_node(0, aeqib_b_val, 0, 0);
                let aeqib_cond = mk_node(20, aeqib_var_a, aeqib_int_b, 0);  // AST_EQ
                let aeqib_then = mk_node(0, 0, 0, 0);                        // AST_INT(0)
                // Push "assertion failed: ==" message bytes (20 chars).
                let aeqib_msg_s = __arena_push(97);                          // 'a'
                __arena_push(115); __arena_push(115);                        // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);     // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);     // 'i' 'o' 'n'
                __arena_push(32);                                            // ' '
                ...
                __arena_push(32);                                            // ' '
                __arena_push(61); __arena_push(61);                          // '=' '='
                let aeqib_str_ast = mk_node(25, aeqib_msg_s, 20, 0);
                let aeqib_args_head = mk_node(17, aeqib_str_ast, 0, 0);
                let aeqib_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
            ...
            } else { if is_assert_ne_ident_bool_form == 1 {
                // K1.F36 (2026-05-27): assert_ne!(IDENT, BOOL_LIT). Sibling of
                // K1.F35; AST_NE cond + "!=" message.
                let aneib_a_s = tok_p2(tok_base, k + 3);
                let aneib_a_l = tok_p3(tok_base, k + 3);
                let aneib_b_s = tok_p2(tok_base, k + 5);
                let aneib_b_l = tok_p3(tok_base, k + 5);
                let aneib_b_val = if is_kw_true_ident(aneib_b_s, aneib_b_l) == 1 { 1 } else { 0 };
                cur_advance(sb);           // IDENT (assert_ne)
                cur_advance(sb);           // !
                cur_advance(sb);           // (
                cur_advance(sb);           // IDENT (a)
                ...
                cur_advance(sb);           // IDENT (true/false)
                cur_advance(sb);           // )
                let aneib_var_a = mk_node(1, aneib_a_s, aneib_a_l, 0);
                let aneib_int_b = mk_node(0, aneib_b_val, 0, 0);
                let aneib_cond = mk_node(21, aneib_var_a, aneib_int_b, 0);  // AST_NE
                let aneib_then = mk_node(0, 0, 0, 0);                        // AST_INT(0)
                // Push "assertion failed: !=" message bytes (20 chars).
                let aneib_msg_s = __arena_push(97);                          // 'a'
                __arena_push(115); __arena_push(115);                        // 's' 's'
                __arena_push(101); __arena_push(114); __arena_push(116);     // 'e' 'r' 't'
                __arena_push(105); __arena_push(111); __arena_push(110);     // 'i' 'o' 'n'
                __arena_push(32);                                            // ' '
                ...
                __arena_push(32);                                            // ' '
                __arena_push(33); __arena_push(61);                          // '!' '='
                let aneib_str_ast = mk_node(25, aneib_msg_s, 20, 0);
                let aneib_args_head = mk_node(17, aneib_str_ast, 0, 0);
                let aneib_panic_name_s = __arena_push(112);
                __arena_push(97); __arena_push(110); __arena_push(105);
                __arena_push(99);
