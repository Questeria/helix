...
on a `@kernel` fn declares a sweep over kernel-launch parameters. Phase-0
records and validates the static sweep specification. The long-term design
is to generate one variant per tuple and use runtime measurement plus cached
dispatch to the fastest variant.
