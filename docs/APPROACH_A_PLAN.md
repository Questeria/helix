...
- **Stage 1.6: expr_type refactor** (added after audit cycle 2 of Stage 1) — replace the three sibling predicates (`is_f32_expr`, `is_f64_expr`, `is_i64_expr`) with one `expr_type(idx, bind, bn) -> i32` returning the type tag (0=i32, 1=f32, 2=f64, 3=i64, 4=bf16, 5=f16, …). Also: change `fn_type_table_lookup` miss sentinel from 0 to -1 to disambiguate "i32" vs "not found"; document the type-tag namespace at the top of `kovc.hx`. **Goal:** N-way cascade goes from O(N) per dispatch site (currently ~12 sites) to O(1). Required before Stage 2 (u8/u16/u32/u64/i8/i16) or the cascade explodes to 30+ levels.
- **Stage 1.7: per-call argument type checking** (added after audit cycle 2 of Stage 1) — extend `fn_type_table` (or add a parallel `fn_param_type_table`) to record each fn's parameter types. At call sites, trap (`ud2`) when an arg's `is_i64_expr` (or future `expr_type`) doesn't match the declared param type. Closes the silent-zero-extend leak in `i64_consumer(-5)` and the parallel f32/f64 cases.

**Latent landmine to address before adding `return`:** the fn-body-vs-ret_ty trap (Stage 1 batch 4) uses `is_i64_expr(fn_body)` to compare against declared ret_ty. If the parser later adds an explicit `return expr;` statement, `is_i64_expr` will need a tag arm for it; otherwise every `fn f() -> i64 { return some_i64; }` will misclassify as i32 and trip a SIGILL. When `return` lands, fold the arm into both the `expr_type` refactor (Stage 1.6) and the AST_RETURN tag handler.
...
### RESOLVED: "Cascade-Depth" Self-Host Bug (was a buffer overrun)

See [docs/BOOTSTRAP_CASCADE_BUG.md](BOOTSTRAP_CASCADE_BUG.md). Root
cause was a 256 KB read-buffer overrun in `read_file_to_arena`, NOT a
problem with cascade depth. Bootstrap source had grown to ~261 KB
with < 1 KB margin against the 256 KB buffer; any new fn tipped it
over and silent truncation propagated into K2 as ud2-patched call
sites that SIGILLed at runtime.

Fix landed 2026-05-07 (commit 29f552e): bumped BUF_SIZE 256 KB -> 1 MB
in both `helixc/backend/x86_64.py` and the four uses in `kovc.hx`'s
`emit_read_file_to_arena_body`. All 5 bootstrap_kovc tests pass.
Verified by re-running the original failing pattern (dead-code arm
