...
| 2 | **Feature-complete + runs (CPU)** | 100% of a language-feature corpus **compiles AND runs** correctly: structs, enums, `match`, generics, traits, closures, floats, all int widths, `grad`, `tile` | 🔶 **CORPUS 15/17 on the self-hosted compiler K2 (2026-06-01, `scripts/feature_corpus.sh`; gated by `scripts/gate_kovc.sh`)**. PASS (15): baseline, scalar-arith, **struct+enum+match** (→129), **payload-enum+match** (→42), **enum+recursion** (→120), **nested PatStruct destructure-in-match** (`dogfood_18`→42 ✅ FIXED), **user-defined `enum Result` + match** (→42), **grad+float** (→42), **i64** cast/compare/negation, **u8/u16** wrap+cast, **i16** overflow, **u64 logical shift** (✅ FIXED). FAIL (2): **i64 division** (5e9/1e9 →0) + **i64 multiply** large (→254) — NOT codegen bugs but a **lexer 32-bit literal-truncation** limitation (`5_000_000_000_i64` wraps before the correct 64-bit divide runs); the 64-bit div/mul machine code is correct; needs lexer/AST literal widening (task #23, deferred). FIXES (each gated: fixpoint K2==K3==K4 byte-identical + GPU PTX unchanged + no regressions): **u64 shift** `ef85342`; **PatStruct `field: subpattern`** parser fix `96c440d3` (the named-struct-pattern parser didn't handle literal/nested/rename field sub-patterns — a feature-completion). Dropped `dogfood_16` from the corpus: it used `Ok`/`Err`/`unwrap_ok` = **deleted Python-frontend intrinsics** (undefined→`ud2`), not a kovc feature — **scope question for user: should `Ok`/`Err`/`Result` be language/stdlib builtins or always user-defined?**. NOT yet in corpus: generics, traits, closures (**no standalone tests — likely post-v1.0; user scope call**), tile (GPU = #3), arrays/more. Criterion NOT met: i64-literal-width + corpus expansion + scope decisions remain. |
