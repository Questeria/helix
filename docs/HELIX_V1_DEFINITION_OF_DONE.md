...
| 2 | **Feature-complete + runs (CPU)** | 100% of a language-feature corpus **compiles AND runs** correctly: structs, enums, `match`, generics, traits, closures, floats, all int widths, `grad`, `tile` | 🔶 **CORPUS 13/17 on the self-hosted compiler K2 (2026-06-01, `scripts/feature_corpus.sh`; gated by `scripts/gate_kovc.sh`)**. PASS (13): baseline, scalar-arith, **struct+enum+match** (→129), **payload-enum+match** (→42), **enum+recursion** (→120), **grad+float** (→42), **i64** cast/compare/negation, **u8/u16** wrap+cast, **i16** overflow, **u64 logical shift** (✅ FIXED — see below). FAIL (4): (i) nested PatStruct destructure-in-match `dogfood_18` — SIGILL 132 (task #21); (ii) Result enum `dogfood_16` — SIGILL 132 (task #22); (iii) **i64 division** (5e9/1e9 →0) + (iv) **i64 multiply** large (→254) — these two are NOT codegen bugs but a **lexer 32-bit literal-truncation** limitation (`5_000_000_000_i64` wraps before the correct 64-bit divide runs); needs AST/lexer literal widening (task #23, deferred). **u64 logical shift FIXED** (`AST_SHR` used arithmetic `sar` for u64; now emits logical `shr` — gated: fixpoint K2==K3==K4 held byte-identical, GPU PTX unchanged, no corpus regressions). NOT yet in corpus: generics, traits, closures (**no standalone tests — likely post-v1.0; FLAG for user whether in scope**), tile (GPU = #3), arrays/more. Criterion NOT met: PatStruct/Result codegen + i64-literal-width + corpus expansion remain. |
