...
| 2 | **Feature-complete + runs (CPU)** | 100% of a language-feature corpus **compiles AND runs** correctly: structs, enums, `match`, generics, traits, closures, floats, all int widths, `grad`, `tile` | 🔶 **STARTER CORPUS 6/8 on the self-hosted compiler K2 (2026-06-01, `scripts/feature_corpus.sh`)**. PASS: baseline-literal, scalar-arith, **struct+enum+match** (`hbs_sample_enum_struct`→129), **payload-enum+match** (`hbs_sample_option`→42), **enum+recursion** (`hbs_sample_recursion`→120), **grad+float** (`gradient_descent`→42). FAIL (compiles but **SIGILL 132 at runtime** — kovc codegen bug): nested PatStruct destructure-in-match (`dogfood_18`), Result enum Ok/Err+unwrap/map (`dogfood_16`). NOT yet in corpus: all int widths (i8/i16/i64/u\*; 976 inline tests in the pre-K4 snapshot `HELIX_STAGE30_COMPILER_SNAPSHOT/helixc/tests/test_codegen.py`), generics, traits, closures (**no standalone tests exist — likely post-v1.0; FLAG for user whether in scope**), tile (GPU = #3). Criterion NOT met: 2 codegen bugs + corpus expansion remain. |
