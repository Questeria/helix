...
| 2 | **Feature-complete + runs (CPU)** | 100% of a language-feature corpus **compiles AND runs** correctly: structs, enums, `match`, generics, traits, closures, floats, all int widths, `grad`, `tile` | 🔶 **CORPUS 12/17 on the self-hosted compiler K2 (2026-06-01, `scripts/feature_corpus.sh`)**. PASS (12): baseline, scalar-arith, **struct+enum+match** (→129), **payload-enum+match** (→42), **enum+recursion** (→120), **grad+float** (→42), **i64** cast/compare/negation, **u8/u16** wrap+cast, **i16** overflow. FAIL (5 — all mirror pre-K4 snapshot tests that PASSED under the deleted Python compiler, so genuine self-host codegen gaps): (i) nested PatStruct destructure-in-match `dogfood_18` — SIGILL 132 (task #21); (ii) Result enum `dogfood_16` — SIGILL 132 (task #22); (iii) **i64 division** (5e9/1e9 →0 not 5); (iv) **i64 multiply** of large values (→254 not 6); (v) **u64 logical shift** (1<<63>>63 →255 not 1) — the three 64-bit-arith bugs = task #23. NOT yet in corpus: generics, traits, closures (**no standalone tests — likely post-v1.0; FLAG for user whether in scope**), tile (GPU = #3), arrays/more. Criterion NOT met: 5 codegen bug-classes + corpus expansion remain. |
