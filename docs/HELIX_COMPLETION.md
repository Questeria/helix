...
3. **Feature corpus** — every corpus program runs through the **fresh K2** to its predicted exit; pass count **never drops below the current baseline** (today **96** = the v1.1 base of 56 [35 v1.0 + 8 H2 generics + 7 H3 traits/closures + 3 H4 guards + 3 H5 i64-literals] grown by the T1/T3 §1.6 gated promotions — >6-arg, L-1 index-store, the L-7 dark-arm + REMAINING-frozen sweeps, the M-1/M-2/L-4 desugars, the M-3/M-5/M-7/L-3 doc-as-bound rows, H-1 collections, H-2 String, the aggregate-return-by-value set, H-4 trait-defaults, M-4 turbofish-enum, M-6 closure-as-arg; the gate banner enumerates the full 96-row breakdown. The baseline rose as T1/T3 added gated tests). `gate_kovc.sh:412,484` (guard `pass < 96`).
...
**✅ POLISH_PASS REACHED (2026-06-03, HEAD `28161af`).** Every §1.6 frozen item is now in a done state
(shipped+gated OR documented-as-bound with a negative test): HIGH H-1/H-2/H-3/H-4 shipped+gated; MED
M-1/M-2/M-4/M-6 shipped+gated, M-3/M-5/M-7 documented-as-bound+neg-test; LOW L-1/L-4 shipped+gated,
L-3/L-5/L-6 documented, and the L-7 `[impl]`→`[proven]` sweep COMPLETE against the frozen denominator
(last 8 arms promoted, commit `628c026`; M-3 8-byte-field bound, commit `28161af`). Gate GREEN on HEAD:
self-host fixpoint K2==K3==K4 byte-identical `9cc8f20b`, GPU+tiled PTX regression OK, CORPUS 96/96,
CHECK_ERR 5/5, GATE_PASS, fence `git ls-files "*.py"` == 1. **T3 is DONE.** Next: §1.4 FINALE (5
consecutive clean independent adversarial audits) → §1.5 officially complete.
...
finale-blocker. The corpus baseline rose accordingly (from the v1.1 base of 56 to the fully-promoted 96). No spec
...
- **H-1** packaged generic collections (`Vec<T>` push/get/set/len/pop + i32 `HashMap`) — *done when* `stdlib/collections.hx` ships and a per-type corpus program is gated. **DONE (2026-06-03):** `stdlib/collections.hx` ships a generic arena-backed `Vec<T>` (`new`/`push`/`get`/`set`/`len`/`pop` with **growth-on-push** — a full Vec relocates into a 2× region and copies) + an i32→i32 open-addressing `HashMap` (`new`/`insert`/`get`/`contains` with **linear-probe collision resolution** + overwrite). State lives entirely in arena cells (`__arena_push`/`__arena_get`/`__arena_set`/`__arena_len`); the generic `Vec<T>`/handle newtypes are read-only field structs (the exact monomorphization surface already gated by `gen_vec_i32`/`gen_vec_f32`), so **NO struct-field STORE → library-level → fixpoint byte-identical** (K2 sha == the H-3 mint `bdff0049…`, verified via the fast inner loop). Gated: `corpus_gen/H1_vec.hx` (cap 2 → push 1..8 forces **two** relocations 2→4→8 → `len==8`&`cap==8` → sum-back via `get`=36 → `set(0,7)` → `pop()==8` → live-sum 34 + popped 8 = **42**) + `corpus_gen/H1_hashmap.hx` (cap 8; keys **3, 11, 19 all hash to bucket 3** — a forced collision resolved by probing into 3,4,5 — + key 6 + an overwrite → `count==4` → `get` each collided key (10,20,5,7) → miss on absent 99 → `contains` present/absent/collided-but-absent(27) → value sum **42**), pass-guard 71→73.
- **H-2** rich `String` (`str_new/str_push_byte/str_eq/str_concat/str_len` + round-trip) — *done when* the String library + round-trip corpus is gated. **DONE (2026-06-03):** `stdlib/string.hx` ships a pure-Helix, arena-backed `String` (`str_new`/`str_push_byte`/`str_len`/`str_byte_at`/`str_concat`/`str_eq`, **growth-on-push** — a full byte region relocates into a 2× region + copies) using the SAME library-level arena pattern as H-1's `Vec<T>`: a `String` is a read-only newtype `String { h: i32 }` wrapping a handle to a 3-cell arena header `[len, cap, data]`, bytes live one-per-cell in the arena, and ALL mutation goes through `__arena_push`/`__arena_get`/`__arena_set`/`__arena_len` — **ZERO struct-field STORE → library-level → fixpoint byte-identical** (K2==K3==K4 == the H-1/H-3 mint `bdff0049…`, full detached gate). Because a string LITERAL used as a runtime value lowers to `mov eax,0` (kovc.hx:9575-9582 — no runtime byte pointer; `__strlen("…")` folds only a literal's byte LENGTH), the rich String is built **byte-by-byte** and every op runs at RUNTIME over arena bytes (nothing constant-folds). The constructors `str_new`/`str_concat` return the arena **handle** (i32), wrapped at the call site as `String { h: str_new() }` — exactly as H-1's `vec_with_cap` returns a handle wrapped `Vec::<i32>{ h: … }` — because **returning a struct BY VALUE** from a function mis-lowers in this from-raw compiler (a v-next codegen gap, the same class as `arm_enum_payload3`'s enum-return-by-value SIGILL); passing a `String` by value as a PARAMETER is fully supported. Gated: `corpus_gen/H2_string.hx` (build "Hel"+"lix" via `str_push_byte` → `str_concat` → "Hellix" whose result starts cap 4 and receives 6 bytes → **forces a grow 4→8** → index byte[0]='H'(72) & byte[5]='x'(120) via `str_byte_at` → `str_eq` proves **EQUAL** (concat vs an independently-built "Hellix"), **UNEQUAL-same-length** ("Hel" vs "lix"), **UNEQUAL-diff-length** (short-circuit), and a **one-byte diff** ("Helliy") → exit = `str_len("Hellix")` * 7 = 6*7 = **42**; pos/neg `str_eq` controls confirm it discriminates, not vacuous), pass-guard 73→74. SCOPE/v-next (honest): `str_new`/`str_concat` return an i32 handle (not a `String` value) until the struct-return-by-value codegen gap is fixed; UTF-8/`char`-aware iteration, substring slicing, and seeding a String directly from a literal's bytes are v-next (the literal-as-runtime-value path is `mov eax,0`).
- **H-3** quality `file:line:col` + message + source-caret diagnostics — *done when* the `check_err` negative corpus asserts compile-time non-zero exit with `path:line:col: message`, full fixpoint green. **DONE (2026-06-03):** kovc now emits a COMPILE-TIME `path:line:col: parse error: unexpected token` diagnostic + a non-zero compiler exit on a malformed program, replacing the legacy "emit an ELF that traps at runtime" model for parse errors. (1) byte→line:col map: the offending token's source byte offset is threaded onto the `AST_ERR` node (tag 99, slot p2) at `parser.hx`'s `parse_primary` unexpected-token catch-all (= `tok_p2` = the lexer token-record `src_start`); `src_off_to_line`/`src_off_to_col` (`kovc.hx`) count newlines/line-start in the source buffer to a 1-based (line, col). (2) wiring: the driver `main` (`drivers/driver_k1{src,input}.hx`) calls `find_first_err_offset(ast_root)` (a fn-list walk mirroring `panic_pass`, descending into the same expression-bearing slots as `walk_for_panic`); on a hit it prints `<path>:line:col: parse error: unexpected token` and returns non-zero, else the normal emit path runs. (3) `check_err` harness: `gate_kovc.sh` `chk_err` compiles 4 error fixtures (`corpus_gen/err_*.hx`) through the fresh K2 and asserts the EXACT `/tmp/k2_in.hx:line:col: …` (hand-verified col against each fixture's bytes: `1:20`, `1:28`, `3:13`, `2:9`) + non-zero exit + no output ELF. FIXPOINT-SAFE: a clean program reaches NO `AST_ERR`, so `find_first_err_offset` returns −1, the emit path is byte-identical, and the self-host fixpoint K2==K3==K4 stays self-consistent (`bdff0049`, the new mint; corpus 71 + GPU PTX regression both unperturbed). Scope: PARSE errors (unexpected-token) carry a real byte offset; validation-pass diags (`diag_arena`, keyed by AST-node index, no byte span) keep the runtime-trap path — a v-next item, not a finale-blocker.
- **H-4** trait DEFAULT methods — *done when* `t1`/`t5` probes are promoted and the fixpoint stays byte-identical. **DONE (2026-06-03, commit `5cbc82c`):** a trait may declare a method WITH a default body; a type that impls the trait but does NOT override that method dispatches to the default, while a type that DOES override it uses its own. (1) **store, not skip:** `parse_trait_decl` (parser.hx) no longer brace-SKIPS the trait body — it walks it method-by-method and records each DEFAULT-bodied method's `fn`-token index + name in a new `tdef_tab` (sb+128/129; the parser state region was extended 32→34 slots so those two are real). The first `{` at trait-body depth after a method's `fn`, before the next `;`/`fn`/trait-`}`, is unambiguously that method's default-body opener (params and a `-> ret` type cannot contain `{`). (2) **dispatch-to-default = synthesis:** `parse_impl_block`, for the `impl Trait for Type` form, after parsing the explicit methods SYNTHESIZES a concrete `<Target>__<method>` for every default method this impl did NOT override, by re-running `parse_impl_method` over the stored default-method token range with the impl's own target type — so `self.field` / `self.other_method()` in the default body resolve against the CONCRETE type, exactly as a hand-written override would. Explicit overrides (whose names are captured into a per-block scratch buffer during the method loop) suppress synthesis → **override wins**. The existing method-call resolution path is UNTOUCHED: `obj.method()` still mangles to `Target__method`, now guaranteed to exist (explicit-or-synthesized). **FIXPOINT-SAFE:** the self-host source uses traits (signature-only methods + explicit impls) but NO default methods, so the synthesis path is inert during self-compilation; the fixpoint sha moves only because parser.hx grew, and **K2==K3==K4 stay byte-identical = `6dbddad812fc858d4dba1b58bdc92a185880728c887190826b967c0042a0e843`** (the new mint; x86-only change → GPU/tiled PTX byte-identical). Gated (corpus 80→82): `corpus_gen/t1_trait_default.hx` (DEFAULT-USED: `trait Greet { fn hello(self)->i32 {42} }` + `impl Greet for P {}` empty → `p.hello()` takes the default → 42) + `corpus_gen/t5_trait_default_mix.hx` (DEFAULT + OVERRIDE: `A` uses the default `hello()`=10, `B` OVERRIDES `hello()`=32 → `a.hello()+b.hello()`=42, proving the override beats the default — B's 32, not the default 10). Existing trait corpus (t2/t7/t7b/t7c) unregressed; full detached gate GATE_PASS, fence `git ls-files "*.py"` == 1. **This is the LAST HIGH §1.6 item — all four HIGH items (H-1 collections, H-2 String, H-3 diagnostics, H-4 trait-defaults) are now shipped+gated.**

**MED — ship+gate OR document-as-bound+negative-test (an explicit recorded decision):**
- **M-1** `for` loops (desugar) — ship+gate (corpus `for`-sum promoted). **DONE (2026-06-03):** the `parse_for` desugar (`parser.hx:16017`) was already implemented + reachable (`for x in a..b` / `a..=b` → `AST_LET_MUT`+`AST_WHILE`+`AST_SEQ`+`AST_ASSIGN`+`AST_ADD`+`AST_LT`/`AST_LE`); promoted with `corpus_gen/M1_for_loop.hx` (exclusive + inclusive + var-bound ranges → 42). Self-host source uses plain `while`, so the fixpoint stays byte-identical (`b7e741c0`).
- **M-2** compound assignment `op=` (desugar) — ship+gate (i32/f32 corpus promoted). **DONE (2026-06-03):** the K1.U/K1.AN compound-assign desugar (`parser.hx:7786`, `x op= e`→`AST_ASSIGN(x, AST_BINOP(VAR(x), e))`) was already implemented for all 10 operators (`+= -= *= /= %= &= |= ^= <<= >>=`); promoted with `corpus_gen/M2_compound_assign.hx` (every operator exercised on runtime values → 42). Fixpoint byte-identical.
- **M-3** 8-byte (f64/i64) generic struct fields — ship+gate (`gen_box_f32`→5), isolated full-fixpoint commit. **Real correctness gap; keep.** **DONE — DOCUMENTED-AS-BOUND + negative test (2026-06-03):** the 4-byte path is shipped+gated (`gen_box_f32`→5; `gen_vec_f32`; the `sret_*field` struct-by-value returns →42). The **8-byte SCALAR field** case (f64/i64/u64) is a documented v1.2 bound: a field's slot is WRITTEN with the full 64 bits at construction (`AST_TUPLE_LIT` stores `mov [rbp+disp],rax` REX.W, kovc.hx:7338), but the field READ (`AST_TUPLE_FIELD`, kovc.hx:7237) emits an 8-byte REX.W load ONLY when `p3==1`, which the parser sets ONLY for nested-struct (child-pointer) fields — NOT for f64/i64/u64 scalar fields (their decl-time type-encoding is `struct_idx == -1`, indistinguishable from i32; parser.hx:2786-2813 + the field-type encoding parser.hx:15495 maps any non-struct non-generic type name through `struct_tab_lookup_idx` == -1). So a wide-field read takes only the LOW 32 bits — and that **silent low-32 truncation is the shared mechanism behind two distinct read paths**, which must be told apart precisely: **(a) i64/u64 wide struct-field reads** AND **(b) — technically — an f64 field whose RAW BITS are cast to i32** (`a.v as i32`, a raw-bits read, NOT float arithmetic). **The genuinely-dangerous, silent-wrong residual is (a) i64/u64** (a field holding `5_000_000_000_i64` reads back `705_032_704` with no trap) — that is the one silent-wrong case, recorded **v-next**. Path **(b) is contained WITHIN the already-disclosed f64-field fail-closed bound**: any f64-TYPED use of the field (`a.v + 0.0_f64`, the dominant case) is i32-typed after the 4-byte read and **fails CLOSED** at the mixed-type guard → ud2/SIGILL **132**, never wrong float math; the only way to observe (b) is the deliberate raw-bits-to-i32 cast, which is the same narrow low-32 read already bounded by the f64-field disclosure (you get the low 32 bits of the IEEE-754 pattern, not wrong float arithmetic). So: **f64 fails closed** for arithmetic (the dominant path) and its sole truncating sub-case (the raw-bits cast) is inside that disclosed bound; **i64/u64 is the genuinely-dangerous 64-bit-looking-but-truncated silent residual.** Negative test `corpus_gen/M3_wide_field_bound.hx` exercises the f64 path and is gated at exit **132** (the loud trap), proving the f64 bound fails closed; corpus guard 95→96. **NO kovc.hx/parser change → self-host fixpoint K2==K3==K4 byte-identical `9cc8f20b`** (the M-6 mint, unmoved); GPU+tiled PTX unperturbed. The full fix (decl-time 8-byte-scalar field detection + a `p3==1` read for them + f64 field-result type-tracking + generic use-site monomorphization — the unimplemented Stage-28.11 **INC-3b** for `struct Box[T]` with `T`=f64/i64) is **v-next**, NOT a finale-blocker — per §1.3 a MED item may close as a documented v1.x bound with a negative test. This is an **explicit, recorded scope decision**.
- **M-4** turbofish-on-enum-constructor — ship+gate (`e2`/`gen_option_i32` turbofish promoted). **DONE — SHIP+GATE (2026-06-03, commit d50e47c):** `EnumName::<T>::Variant(payload)` (and the unit `::Variant`) now construct correctly. ROOT CAUSE: the leading `IDENT::<` matched the generic-fn turbofish detector (`t3 == LT`), whose branch then looped scanning for `(args)` after the closing `>`, found `::` instead, and **HUNG the compiler** (rc 124 compile timeout); the bare `Opt::Some(42)` form already worked. FIX (FRONTEND PARSE, `helixc/bootstrap/parser.hx` `parse_primary`): `is_enum_turbofish` detects `EnumName::<...>::Variant` when the leading IDENT is a registered enum (angle-balanced scan past `>` then `:: IDENT`), MASKS the generic-fn turbofish flag, and routes construction to the SAME `AST_TUPLE_LIT` enum-construct path as the bare form (`is_enum_unit`/`is_enum_payload`), skipping the `::<T>` type-arg segment — the bootstrap is type-erased so the monomorph carries no runtime tag (identical to bare construction). Non-enum turbofish `id::<T>(...)` untouched (the scan only runs when the IDENT is an enum). Tests: `corpus_gen/M4_turbofish_enum.hx`→42 (turbofish payload `Some(40+k)` extracted via match + turbofish unit `None` selecting the None arm; runtime-derived) + `gen_option_i32.hx`→42 (charter probe). GATE GREEN: self-host FIXPOINT K2==K3==K4 byte-identical `0f846aea…` (sha moved off the H-4 mint because parser.hx changed; the self-host source never uses turbofish enum ctors), GPU PTX byte-identical, CORPUS 84/84 (guard 82→84), fence=1.
- **M-5** bare generic call at non-i32 — ship arg-type inference (`gen_bare_f32`→3) **OR** document "explicit turbofish required for non-i32 scalar generics" as a v1.1 bound + negative test. **DONE — DOCUMENTED-AS-BOUND (2026-06-03):** verified the bound is real: a BARE (no-turbofish) call to a free generic fn at a non-i32 scalar type does NOT infer the type argument — the monomorphizer defaults the generic param to i32, so `id(3.0_f32)` returns **0** (the f32 bit pattern is reinterpreted as i32), not 3. The supported idiom is EXPLICIT TURBOFISH (`id::<f32>(3.0_f32)`→**3**, `add2::<f32>(2.0,3.0)`→**5**, both verified) or impl-block monomorphization (`Box::<f32>`). Negative/bound-proving test `corpus_gen/M5_bare_generic_bound.hx` asserts the bare form yields 0 (gated at exit 0). A full bare-call type-inference pass is **v-next**.
- **M-6** higher-order closures (closure-as-argument) — ship+gate (`t6`/`t6b`→42). **DONE — SHIP+GATE (2026-06-03):** passing a (non-capturing) closure AS AN ARGUMENT to a higher-order fn that invokes it now works. ROOT CAUSE (CODEGEN/lowering): closures have no runtime object — a closure literal lowered to `AST_INT(0)` and the binding was a parse-time-only `cl_var_tab` entry (so `let g=…; g()` rewrote to a direct `__closure_<id>(…)` named call). In ARGUMENT position the literal therefore passed the integer **0** (fn pointer lost); the synthesized `__closure_<id>` fn existed but was never referenced, and inside the callee `f(x)` indirect-called through the param's value → `call *0` → **SIGSEGV(139)** (confirmed by disasm: `mov $0x0,%eax` at the literal + `call *%r11` in the callee). FIX (`helixc/bootstrap/parser.hx` `parse_closure_lit`): for a **non-capturing** closure, return `AST_VAR(__closure_<id>)` instead of `AST_INT(0)` — codegen's pre-existing A2a arm emits `lea rax,[rip+__closure_<id>]` (a real fn pointer), which flows as the arg, and the callee invokes it via the pre-existing A2b indirect-call (`call r11` through the bound param). The synthesized fn is spliced into the fn list before the `fn_type_table` pre-pass, so A2a resolves it. The local by-name path is unchanged (`cl_var_tab` still wins for `g(…)`; the fn pointer in g's slot is unused there). Tests: `corpus_gen/M6_closure_arg.hx`→42 (a closure passed as an arg & invoked — incl the SAME closure twice, two invocations, and TWO distinct closure args), `t6_closure_arg.hx`→42 (charter probe `apply(|y| y+1, 41)`, pre-fix SIGSEGV), `M6_capture_regression.hx`→42 (a capturing closure by-name still works). GATE GREEN: self-host FIXPOINT K2==K3==K4 byte-identical, GPU PTX byte-identical, CORPUS 87/87 (guard 84→87), fence=1. **RESIDUAL v1.2 BOUND (honest):** a **CAPTURING** closure passed BY VALUE as an argument is NOT supported — the synthesized fn takes the captures as extra LEADING params (`__closure_<id>(cap0,…,params…)`) so a flat fn pointer cannot carry the captured environment. Capturing closures keep the `AST_INT(0)` value, so passing one by value fails LOUD (`call *0` → SIGSEGV) rather than silently dropping the capture; the by-name local-call path (which injects captures positionally) is the supported idiom for captures. A first-class heap closure object (env + fn-ptr) is **v-next**.
- **M-7 → DOCUMENT-AS-BOUND (CUT from ship).** Per §b: cross-module `pub`/private enforcement + module-scoped types + an external-file loader is an L-sized language feature, touches the self-host fixpoint, and is **not needed for the stated vision** (single-file + function-mangling modules already work). **Done-criterion = document the current module semantics as the v1.x bound + add ONE negative privacy test.** A full privacy/visibility pass is **v-next**, only if the user specifically asks for cross-module privacy. **DONE — DOCUMENTED-AS-BOUND (2026-06-03):** the current semantics: `mod name { ... }` function-mangles inner fns (`hidden`→`secret__hidden`) reachable via the `name::fn()` path, and `pub` is parsed as a **no-op** — there is **NO privacy enforcement**: a private (non-`pub`) module fn is freely callable across the module boundary (Rust rejects with `error[E0603]`). Negative privacy test `corpus_gen/M7_privacy_bound.hx` calls a private `secret::hidden()` cross-boundary; the bootstrap accepts it and runs the body → **42** (gated), proving the absence of privacy enforcement. A visibility pass + filesystem module loader is **v-next**.
...
- **L-3** match-exhaustiveness — **non-exhaustive `match` as a compile error is nice-to-have, NOT vision-blocking**: ship the negative-corpus compile-error check **OR** document as a v-next bound. Not a finale-blocker on its own. **DONE — DOCUMENTED-AS-BOUND + negative test (2026-06-03):** verified there is NO exhaustiveness checking — a `match` that omits a variant is accepted (Rust rejects: "non-exhaustive patterns") and runs the covered arm normally. Negative/bound-proving test `corpus_gen/L3_nonexhaustive_bound.hx` (payload-enum match dropping the `Err` arm, scrutinee on `Ok(42)`) → **42** (gated), proving the compiler does not reject non-exhaustive matches. A proper exhaustiveness checker is **v-next**.
- **L-4** `&&`/`||` short-circuit (desugar) — boolean-logic corpus. **DONE (2026-06-03):** the K1.M-fix short-circuit desugar (`parser.hx:2267`) was already implemented + reachable: `a && b`→`if a { b } else { 0 }`, `a || b`→`if a { 1 } else { b }`, both lowering to `AST_IF` (which branches — `kovc.hx:8933` emits `test rax,rax` + a conditional jump over the untaken arm, so the RHS in the not-taken branch is **never executed**). Promoted with `corpus_gen/L4_short_circuit.hx`, which PROVES RHS-skip via arena-slot side-effect channels: `false && mark(s)` / `true || mark(s)` leave their channel at 0 (RHS skipped) while `true && mark(s)` / `false || mark(s)` set theirs to 1 (RHS ran) → 42. Self-host source uses nested `if`, so the fixpoint stays byte-identical.
- **L-5 → PERMANENT DESIGN CHOICE (FIRM, not a maybe).** References/borrows `&T`/`&mut T` are **out of scope for complete-for-now**: a borrow checker is a multi-month language project. **Decision: the arena-index-as-handle model is the permanent v1.x idiom.** Done-criterion = document the arena-handle decision in the spec. **Not implemented; not a finale gate.** **DOCUMENTED (2026-06-03):** verified the honest behavior — `&` is primarily bitwise-AND (`12 & 10`→8); a read-only `let r = &x; *r` happens to pass the value through (→ correct read), but **`&mut` does NOT model aliasing**: `let r = &mut x; *r = 42; x` leaves `x` unchanged (→40, the write is dropped). So there is no real borrow/pointer semantics; the **arena-index-as-handle idiom is the permanent mutation channel** (the gated `L1_index_store`/`vec_arena` rows are the supported aliased-write pattern). Doc-only; not gated.
- **L-6 → DOCUMENT-AS-BOUND.** `const`/`static` real semantics + const-folding — nice-to-have, not vision-blocking. Done-criterion = document the parsed-erased semantics as the v1.x bound (implement only if cheap and fixpoint-safe). Not a finale-blocker on its own. **DOCUMENTED (2026-06-03):** verified — simple integer `const` DOES produce a usable value via substitution (`const N: i32 = 42; N`→42; `const A=20; const B=22; A+B`→42), better than "parsed-erased." The genuine v1.x bound is the absence of **general const-folding, `const` in type/array-size position, and `static` mutable semantics** — those are v-next. Doc-only; not separately gated (the working integer-const path rides the existing corpus).
- **L-7** `[impl]`→`[proven]` sweep — **BOUNDED: the `[impl]` set is enumerated ONCE here and frozen.** The frozen denominator is exactly: nested block comments `/* */`; hex/bin/octal + `_` separators; char literals; `i8`/`i16`/`u32` widths; `bf16`/`f16` float-lit shapes; tuple structs; `continue`; early `return`; `~e`/`!e` unary; index-store (L-1); reflection stubs (`__hash_i32`/`__trace_*`/`__helix_*`, `kovc.hx:2120,4569`). *Done-criterion =* one corpus row per FROZEN `[impl]` item converting it `[impl]`→`[proven]`, **OR** documenting it as a deliberate no-op/design choice (reflection stubs may be documented-as-no-op). **Any `[impl]` feature discovered AFTER this freeze is a v-next row, NOT a finale-blocker** — the sweep can no longer "always find one more." **PROGRESS (2026-06-03):** `~e`/`!e` unary (`arm_neg`/`arm_bnot`/`arm_not`) + `i8`/`u32` widths (`arm_i8_width`/`arm_u32_width`) promoted `[impl]`→`[proven]` (gated in `gate_kovc.sh`, corpus 60→65, fixpoint byte-identical `b7e741c0`); the remaining frozen items (nested block comments, hex/bin/octal+`_`, char lits, `i16` [already gated via `i16_ovf`], `bf16`/`f16`, tuple structs, `continue`, early `return`) await probes/promotion; reflection stubs document-as-no-op. **SWEEP COMPLETE (2026-06-03, commit `628c026`):** the last frozen L-7 arms are now promoted `[impl]`→`[proven]` (8 corpus rows, all run through the fresh K2 to their predicted exit): `arm_block_comment`→42 (nested `/* /* */ */`), `arm_radix_lits`→42 (hex `0x2A`/`0xFF_FF`, bin `0b10_1010`, oct `0o52` WITH `_` seps), `arm_char_lit`→42 (`'*'`=42, `'\n'`, `'0'`), `arm_continue`→42, `arm_early_return`→42, `arm_tuple_struct`→42 (`Pair(i32,i32)` + `.0`/`.1`), `arm_bf16_f16_decl`→42 (bf16/f16 LITERAL decl, truncated bit pattern emitted), and `arm_bf16_arith_bound`→**132** (bf16/f16 ARITHMETIC is storage-only with no x86 baseline hardware → `ud2`/SIGILL, a DOC-AS-BOUND that fails closed, never silent-wrong). `i16` was already gated (`i16_ovf`); reflection stubs (`__hash_i32`/`__trace_*`/`__helix_*`) are documented-as-no-op (a deliberate design choice per the done-criterion). **NO kovc.hx/lexer/parser change → self-host fixpoint K2==K3==K4 byte-identical `9cc8f20b` (the M-6 mint, unmoved); GPU+tiled PTX unperturbed; corpus 87→95.** **Every frozen L-7 `[impl]` item is now `[proven]` (a gated corpus row) OR a documented no-op/bound — the L-7 sweep is COMPLETE against the frozen denominator.** **arm_enum_payload3 FIXED + PROMOTED v-next→gated (2026-06-03, commit `e3c7755`):** the enum-return-by-value SIGILL was a real x86 codegen bug — a struct/enum returned BY VALUE is a POINTER to a slot-run built in the CALLEE's frame, which the epilogue reclaims (dangling pointer), compounded by a 32-bit truncating let-store of that pointer and i32-direct match typing for enum returns. Fixed in `kovc.hx` (AST_CALL copies the run into the CALLER frame the instant the call returns + 64-bit let-store of aggregate bindings) + `parser.hx` (enum returns tagged pointer-rep `100+8+enum_idx`; `fn_ret_struct_tab` re-enabled so `let x=struct_call(); x.field` resolves the field). `corpus_gen/arm_enum_payload3.hx`→**42** now gated; also added `sret_1field`/`sret_2field`/`sret_3field`/`sret_5field` (1/16/>16/40-byte struct return + every-field read) + `eret_option` (2-variant enum return + match), all→42. Corpus 74→80; **full fixpoint K2==K3==K4 byte-identical `0c5363ba`** (sha moved only because parser.hx source grew — self-host returns no aggregates so the new path is inert during self-compile); GPU PTX regression unperturbed. This also closes the **H-2** struct-return-by-value gap (the String constructors `str_new`/`str_concat` could now return a `String` value, though they keep the i32-handle idiom for now).
...
> **M-4. Turbofish-on-enum constructor** (`Opt::<T>::Some(x)`). (S) Turbofish struct-literal fixed (`parser.hx:7350-7393`); enum-variant-path turbofish still **hangs the parser**. Bare construct works+gated (`e5`/`e6`→42); probe `e2_construct.hx` NOT gated. Fix the `IDENT::<T>::Variant` path; promote `e2`/`gen_option_i32` turbofish. Generics-only → fixpoint-safe. **VERIFIED NOT-PROMOTABLE (2026-06-03):** ran `e2`/`gen_option_i32` through fresh K2 (`b7e741c0`) — both **silent COMPILE-FAIL** (K2 emits nothing, no diagnostic). This is a **real frontend gap requiring a kovc.hx parser change**, NOT a fixpoint-safe promote-existing-probe — the §1.6-order "M-4 fixpoint-safe" label is the path, not the current state.
...
> **M-6. Higher-order closures** (closure-as-fn-argument). (S/M) Capture works+gated (`t3/t4/t8`→42); closure-as-argument deferred; fn-typed params type-erased to i32 (`kovc.hx:1018,9169`). Probes `t6_closure_arg.hx`/`t6b_closure_arg.hx` NOT gated. Implement fn-pointer passing; promote `t6`/`t6b`→42. Closures-only → fixpoint-safe. **VERIFIED PARTIAL (2026-06-03):** through fresh K2 (`b7e741c0`), `t6b` (locally-bound closure `let g=|y| y+1; g(20)+g(20)`) **PASSES → 42**, but `t6` (closure passed *as an argument* `apply(|y| y+1, 41)`) **compiles then SEGFAULTs at runtime** (bad call codegen). So M-6 is **half-broken: needs a real kovc.hx codegen fix for closure-as-argument** before `t6` can be gated; `t6b` alone is gateable but does not close the item.
...
> **L-7. `[impl]`→`[proven]` truthfulness sweep — BOUNDED, FROZEN DENOMINATOR (per §1.6).** The `[impl]` set is enumerated ONCE in §1.6 and frozen: nested block comments `/* */`; hex/bin/octal + `_` separators; char literals; `i8/i16/u32` widths; `bf16/f16`; tuple structs; `continue`; `return` early-exit; `~e`/`!e` unary; index-store (L-1); reflection stubs that return 0 (`kovc.hx:2120,4569` — `__hash_i32`/`__trace_*`/`__helix_*`). *Gate:* add one corpus row per **frozen** `[impl]` feature, converting each `[impl]`→`[proven]`; for reflection stubs, decide implement-or-document-as-no-op. **Any `[impl]` feature discovered AFTER the freeze is a v-next row, NOT a finale-blocker** — the sweep cannot "always find one more." Low-risk, high-trust; tightens the spec honesty legend. **This sweep is part of POLISH_PASS §1.3.** **PROGRESS (2026-06-03):** 5 dark-arm probes promoted+gated (corpus 60→65, fixpoint byte-identical `b7e741c0`): `arm_neg`/`arm_bnot`/`arm_not` (`~e`/`!e` unary, AST_NEG 9/BNOT 26/NOT 27) + `arm_i8_width`/`arm_u32_width` (`i8`/`u32` widths — `u8`/`u16`/`i16` were already gated). Each loop-accumulates a runtime value so the arm is dynamically exercised (un-foldable). **DARK-ARM BUG FOUND (not promoted, fail-closed):** `arm_enum_payload3.hx` (3-payload enum **returned by value from a function** then matched) **compiles but SIGILLs at runtime** — isolated: the inline `Tri::C(30)` + 3-arm match works (→42) but enum-return-from-fn through 3 variants is a **real codegen bug** → a v-next fix, NOT gated. (`arm_mod`/`arm_shr`/`arm_str_lit`/`arm_struct_field`/`arm_cast_chain` also pass clean but their arms — mod/shr/str-lit/struct-field-read/cast — are **not in the frozen L-7 denominator**, so they are NOT added to the finale gate per the scope freeze.)
...
- The GPU path does **real** tiled + Tensor-Core compute (`.shared`/`bar.sync`/`cp.async`/`mma.sync` genuinely emitted), **within ~2–3× of cuBLAS** on the reference box, correctness-gated against cuBLAS + a CPU oracle, and the capstone transformer re-trains on it with **2% loss parity (the hard gate, held at ~0%)** and an end-to-end speedup of **7.03–8.70× across framings — ≥10× is NOT met** (Amdahl-bound by the f32-SMEM cp.async GEMM tier; TF32 Tensor Cores are a confirmed dead-end on this GPU at 0.97× the f32 GEMM). See the §1.2 item-5 verdict above (~line 560) and `docs/HELIX_GPU_PERF_RESULT.md`.
...
== 1 `.py`, corpus baseline 96 [rose from the v1.1 base of 56 as T1/T3 added gated tests], bundle + tag present). Verify the live HEAD with `git rev-parse --short
