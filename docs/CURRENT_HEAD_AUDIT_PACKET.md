...
| committed `.c`/`.h` + LOC | `git ls-files "*.c" "*.h" \| wc -l` ; `\| xargs wc -l` | **28 / 18 128** — the v1.3 trusted toolchain (24 / 15 605, Category A from-raw ladder + seed) is unchanged; +4 Category-B host tools = `helixc/runtime/gpt2_infer.c` (post-v1.3 GPT-2 demo launcher, CUDA-FFI), `helixc/runtime/cpu_host.c` (579 LOC, the post-v1.3 CPU no-ptxas demo launcher — CUDA-FREE byte-movement harness, ZERO arithmetic on the trust path), `helixc/runtime/gpt2_tok.c` (659 LOC, the offline byte-level BPE tokenizer that makes the demo's production data path Python-free — byte/id bookkeeping only, ZERO arithmetic on the trust path) and `helixc/runtime/gpt2_pack.c` (345 LOC, the offline safetensors→`.weights` importer — byte-movement only, ZERO arithmetic on the trust path); `cuda_launch.c` grew +273 (GPU verify modes). The tokenizer's Unicode range tables live in `helixc/runtime/gpt2_unicode_ranges.inc` — a generated DATA `.inc`, NOT a `.c`/`.h`, so outside this fence (like a `.hx`) |
