...
- **GAP — general-purpose collections & rich strings**: there is **no packaged `Vec`/`HashMap`/list type** and **no rich string type** (only arena-byte `&str`). But collections are **demonstrably user-implementable on the arena** — `scripts/feature_corpus.sh`'s `vec_arena` POC builds a growable int `Vec` (`vec_new`/`vec_push`/`vec_get`/`vec_len` over the arena) and runs correctly (→45). The compiler itself is written this way. So this is a **library** gap (no packaged collections), not a **language** gap, and **the capstone does not need them**.
