...
| macros | ⚠️ PARTIAL | `IDENT!(...)` parses as no-op call (K1.CB 2026-05-26). K1.F22 (2026-05-27) ships the bootstrap's FIRST real macro expansion: `panic!("msg")` now rewrites at parse-time inside K1.CB's is_macro_call branch to AST_CALL(panic, str_arg), using the existing panic builtin codegen (K1.AE/AH/AI: sys_write the message then ud2). Scope is intentionally narrow -- only the exact `panic!(STR)` shape; multi-arg Rust-style panic!("fmt", x), and other IDENT!(...) macros (println!, vec!, assert_eq!, etc.) still hit the no-op-skip path until a more general macro engine lands. Pinned via test_bootstrap_kovc_k1f22_panic_macro_expansion_self_host (panic!("oops"); 42 -> rc=132). |
