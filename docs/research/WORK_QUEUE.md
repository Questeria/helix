...
### 1. Bind pattern variables into match-arm body scope (S)
...
### 2. Exhaustiveness checker — boolean and unit (S)
...
### 3. Guard typecheck must be bool, body types must unify (S)
...
### 4. Or-patterns `a | b | c` in parser + AST (S)
...
### 5. Range patterns `0..10` and `0..=10` (S)
...
### 6. Codegen Match → chained `if`-let lowering (M)
...
### 7. Match in autodiff: passthrough scrutinee, propagate gradients to selected arm (M)
...
### 8. Memoize differentiate() by body hash (S)
...
### 9. Replace Quote handle (currently Python-`hash() mod 64`) with content-addressed slot allocation (M)
...
### 10. Add `ast_hash` round-trip test for every node kind (S)
...
### 12. Add `__pow(x, n)` integer exponent builtin + AD chain rule (S)
...
### 15. Const-fold pass: extend algebraic identities to `x*1, 1*x, x/1, x+0, 0+x` (S)
...
### 16. Did-you-mean for misspelled stdlib calls (S)
...
### 19. Better Match-related error spans (S)
