...
3. **kov-libc — write our own** in M2-Planet C subset (~200–500 LOC). Avoids GPL-3.0 contagion from M2libc into kovc-bootstrap.
4. **kovc-bootstrap.c — write our own** in M2-Planet C subset (~5–10 kLOC).
5. **kovc — self-host in Kov.**
...
| **(c) Adopt through M2-Planet, write kovc-bootstrap.c** | 1 PM | 2 PM | 4 PM |
| (c) + write own kov-libc to escape GPL-3 | +0.5 PM | +1 PM | +2 PM |
...
6. **License contagion**: M2libc is GPL-3.0; if we link it into kovc-bootstrap, the binary inherits GPL-3.0. Solution: write our own kov-libc shim (~200–500 LOC).
7. bootstrap-seeds repo says "NEVER TRUST ANYTHING IN HERE" — audit ASCII vs binary once, commit both, pin SHA256.
8. M2-Planet's test corpus is shallow — write our own conformance tests for kovc-bootstrap features.
