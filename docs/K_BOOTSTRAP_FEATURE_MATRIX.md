...
| `Splice(handle)` (call form) | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 29 2026-05-26: same as Quote -- bootstrap has `Splice` registered at slot 119 with bytes `83 112 108 105 99 101` (capital S). Direct probe `fn main() -> i32 { Splice(0) }` returns 0 (the cell-table OOB-safe path returns 0 since cell[0] is zero-initialized BSS). Bootstrap implements full bounds-checked load from the arena's reflection-cell region at disp_base=8388356. Python NotImplementedError on the direct-compile probe) |
