...
| `panic("msg")` builtin | ✅ (`panic_pass.py`) | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 20 + K1.AE 2026-05-25: `panic("msg")` compiles + traps + NOW PRINTS THE MESSAGE. K1.AE added a dedicated arm in try_emit_builtin_call: when AST_CALL's callee name byte-matches "panic" and the first arg is AST_STR_LIT (tag 25), codegen emits `lea rsi, [rip + msg]` (via new emit_lea_rsi_rip_placeholder + str_table_add) + `mov edi, 2` (fd=stderr) + `mov edx, len` + `mov eax, 1` (sys_write) + `syscall` + `ud2` (26 bytes total). Python emits "panic[id]: msg\n" to stderr via sys_write + sys_exit; the bootstrap omits the "panic[id]: " prefix and uses ud2 instead of sys_exit, but the parity-relevant property -- "user sees the panic message before the program aborts" -- now holds in both compilers. Pinned via 3 self-host tests: `_panic_traps` (rc=132), `_panic_unreachable_after` (post-panic 42 never reached), and `_panic_prints_message` (stderr contains the message)) | PARITY |
