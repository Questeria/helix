...
| `impl Type { methods }` | ✅ (`flatten_impls.py`; with caveat) | ❌ | KOVC-MISSING (K1.F5-discovery 2026-05-26 LOCALIZED: bootstrap parses `impl Type { fn method(self: T) }` (K1.BF) and emits the mangled fn `<TypeName>__<MethodName>` via mangle_impl_method (parser.hx:9933). Static-path call `P::get_const()` ALSO works (bs=42). But the DOT-CALL syntax `p.get()` SIGILLs. Probes localized to parse_primary's `.IDENT(` postfix branch (parser.hx:2272+): the method-call detection uses `var_type_tab_lookup` (scalar types only), which misses struct-typed vars stored in `var_struct_tab`. ATTEMPTED FIX (reverted same tick): add a parallel `var_struct_tab_lookup` check + `mangle_impl_method` call. Fix branch was reached structurally (no K2 regression, 71/71 PASS) but `p.get()` still SIGILLs -- the bootstrap-built bootstrap may not flow through the new branch as expected. Hypothesis for next tick: `var_type_tab` may ALSO track struct vars with the 100+struct_idx encoding, so `method_lhs_ty >= 0` triggers the scalar-method-call branch which then mangles incorrectly via ty_tag_push_name (designed for scalar tags 0-11, not struct tags 100+). Next-tick approach: probe-trap at scalar branch entry to confirm which branch is reached, then either fix ty_tag_push_name to handle struct tags OR move the struct-method-call check BEFORE the scalar one. Row stays KOVC-MISSING; investigation will continue) |
