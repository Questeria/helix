...
| Mixed-type binops (e.g. i64+i32) | ✅ (implicit conversion) | ✅ FUNCTIONAL PARITY for signed i64<->i32 ADD/SUB/MUL (K1.F8 + K1.F8b 2026-05-27): both directions sign-extend the i32 operand to i64 and emit a 64-bit op. K1.F8 closed (l_i64=1, r_i32=0): the i32 in rcx is widened via movsxd_rcx_ecx after the existing mov-rcx step. K1.F8b closed (l_i32=0, r_i64=1): the mov-rcx step is patched to use a 64-bit copy (mov_rcx_rax_64) when r is i64 so rcx has the full i64 (the original 32-bit copy lost the high 32); then after pop rax, movsxd_rax_eax sign-extends the i32 LHS to full i64; then 64-bit add/sub/imul. Two new helpers `emit_movsxd_rcx_ecx` / `_rax_eax` (3 bytes each, REX.W 63 + ModRM). expr_type for AST_ADD / SUB / MUL returns tag 3 (i64) for BOTH `(3, 0)` and `(0, 3)` operand-tag pairs so the body-vs-ret-ty trap 14001 doesn't fire on `-> i64 { i64 op i32 }` or `-> i64 { i32 op i64 }`. Verified by 7 self-host probes (3 reverse + 2 forward + 2 regression for i64+i64 and i32+i32). REMAINING gaps in this row (not yet closed): unsigned-mixed (u64 vs u32 still traps 30/31 series), float mixed (f64+f32 still traps 10/11/40/41), DIV / MOD operators not patched. These don't appear in the bootstrap-compileable self-host source so the i64<->i32 close suffices for the K-track) |
