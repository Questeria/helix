...
| `impl Type { methods }` | ✅ (`flatten_impls.py`; with caveat) | ❌ | KOVC-MISSING (K1.F-discovery batch 28 2026-05-26 RE-AUDIT: confirmed Python works for `impl T { fn method(self: T) -> ... }` shape -- `let p = P{v:42}; p.get()` returns 42 in Python. But Python REJECTS the real-Rust shorthand `fn method(&self)` with `ParseError: expected IDENT (got AMP '&')`. So Python's `flatten_impls` is genuinely usable on the limited "self: T" shape; the `&self` form is a Python-side gap. Bootstrap kovc parses both shapes (K1.BF / K1.DV) but the method-DISPATCH at call site `p.get()` SIGILLs -- the real remaining work is the bootstrap-side dispatch, not just parsing. This row stays KOVC-MISSING; matrix description sharpened) |
