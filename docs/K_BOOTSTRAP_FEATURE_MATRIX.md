...
| `__trace_event` (trace ring buffer) | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F3 2026-05-26: bootstrap now registers `__trace_event` as a variadic-tolerant no-op stub at slot 165 returning 0 in eax. Direct probes: `__trace_event(); 42` returns 42 and `__trace_event(1); 42` also returns 42. Python's compile-and-run path still raises NotImplementedError, but bootstrap accepts and discards the call; both compilers' exit codes match for the bootstrap-compileable subset. Real trace-ring-buffer behavior is a Phase-1 observability follow-up) |
