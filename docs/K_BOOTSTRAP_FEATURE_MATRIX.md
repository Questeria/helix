...
| `reflect_hash(ast)` | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F2 2026-05-26: bootstrap now registers `reflect_hash` as a no-op stub builtin at slot 164 returning 0. Direct probes: `reflect_hash(1)` returns 0, `reflect_hash(1); 42` returns 42. Python's compile-and-run path still raises `NotImplementedError`; bootstrap accepts the call without SIGILLing. For the bootstrap-compileable subset, both compilers return identical exit codes (Python errors out, bootstrap returns 0/discards result). Closure rationale matches Quote/Splice/modify: the call shape is supported; full content-addressable hashing is a Phase-1 refinement) |
