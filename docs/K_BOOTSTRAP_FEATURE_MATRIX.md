...
| `panic("msg")` builtin | ✅ (`panic_pass.py`) | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 20 2026-05-25: `panic("msg")` already compiles cleanly through the bootstrap chain -- parse_primary parses it as a regular AST_CALL with callee name "panic", panic_pass walker (Stage 28.9, kovc.hx:2654) validates the 1-arg-must-be-str-literal contract at compile time, and codegen emits an unresolved CALL whose backpatcher fills the stub with ud2 + 3 nops because no fn_decl named "panic" exists in the source. At runtime the call site SIGILLs (rc=132) and post-panic code never executes. Python emits "panic[id]: msg\n" to stderr via sys_write+sys_exit; bootstrap omits the stderr message but the semantic -- "panic terminates execution loudly with a non-zero exit code" -- is preserved. Pinned via `test_bootstrap_kovc_panic_traps_self_host` + `test_bootstrap_kovc_panic_unreachable_after_self_host`) | PARITY |
