...
| `f16` literal | ✅ | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 28 2026-05-26: `let x = 1.0_f16; 42` returns 42 in BOTH compilers. Python helixc lowers the f16 literal via the bf16 codepath (truncated f32 mantissa). Bootstrap kovc accepts `_f16` per K1.BH as bf16-shaped and emits the same bf16 codepath. Bit-accurate IEEE-754 half precision is a separate full-fix item; for the bootstrap-compileable subset where the f16 value flows through arithmetic without observing the exact bit pattern, both compilers behave identically. Vacuously satisfied) |
...
| Mixed-type binops (e.g. i64+i32) | ✅ (implicit conversion) | ✅ FULL PARITY across signed/unsigned/float (K1.F8 through K1.F9-fix 2026-05-27 + K2.T matrix-honesty sweep): all 5 arith ops (ADD/SUB/MUL/DIV/MOD; MOD float traps by design -- no SSE remainder) widen in both directions across signed i64<->i32 (K1.F8/F8b/F8c via movsxd helpers), unsigned u64<->u32 (K1.F8d via zero-ext through mov_ecx_eax, no movsxd needed since u32 is already zero-extended), and float f64<->f32 (K1.F9 + K1.F9-fix via cvtss2sd helpers `emit_cvt_f32_in_rax_to_f64` / `_rcx_to_f64`; K1.F9-fix added the missing `r_d == 1` mov-rcx leg in AST_ADD that K1.F9 had given SUB/MUL/DIV but not ADD). All paths use K3.B-style exactly-i32/u32/f32 guards so non-exact-type r-side still traps loudly (audit-clean signal in K3.E + K3.F). Mixed-type comparison widening is row 76 below |
| Float arithmetic (f32) | ✅ | ✅ (via `__fadd/sub/mul/div/neg` SSE builtins) | PARITY |
| Float arithmetic (f64) | ✅ | ✅ | PARITY |
| Mixed f32/f64 arithmetic | ✅ | ✅ FUNCTIONAL PARITY (K1.F9 `f290393` + K1.F9-fix `8ea2f66` 2026-05-27: SSE cvtss2sd widening across ADD/SUB/MUL/DIV in both directions. Two new helpers `emit_cvt_f32_in_rax_to_f64` / `_rcx_to_f64` (13 bytes each: movd xmm/eax + cvtss2sd + movq rax/xmm). The K1.F9-fix closed an ADD-reverse miscompile: the original commit added the `r_d == 1` mov-rcx leg to SUB/MUL/DIV but missed AST_ADD, so `f32 + f64` saw a truncated f64 in rcx. K3.B-style exactly-f32 (tag 1) guard ensures non-f32 r-types still trap. MOD still traps by design -- no SSE remainder instruction. Permanent self-host test pinned in test_codegen.py::test_bootstrap_kovc_k1f9_mixed_f32_f64_self_host. Verified via 8 mixed-direction probes) | PARITY |
| Unary `-` (`AST_NEG`) | ✅ | ✅ | PARITY |
| Unary `!` (`AST_NOT`) | ✅ | ✅ | PARITY |
| Unary `~` / `AST_BNOT` | ✅ | ✅ | PARITY |
| Bitwise `& | ^` | ✅ | ✅ (BAND/BOR/BXOR) | PARITY |
| Shifts `<< >>` | ✅ | ✅ (AST_SHL/AST_SHR) | PARITY |
| Comparisons `< > <= >= == !=` | ✅ | ✅ FULL PARITY incl. mixed-type widening (K1.F11/F12/F13/F14 2026-05-27): all 6 comparison ops handle mixed signed i64<->i32 (F11 LT, F12 batch GT/EQ/NE/LE/GE), unsigned u64<->u32 (F13 6-site batch), and float f64<->f32 (F14 6-site batch using cvtss2sd + ssen_*_dbl helpers). EQ/NE share the i64 helper for both i64 and u64 paths (bitwise (in)equality is signedness-agnostic). Audit-clean signal recorded in K3.F for the silent-failure axis (6 fail-closed probes confirm exactly-type guards trap loudly on signed/unsigned mismatch, int-as-float-bits, etc.). K2 corpus probes p94-p107 (signed) and p108-p119 (unsigned); float mixed-cmp parity is pinned bootstrap-only in test_codegen.py::test_bootstrap_kovc_k1f14_mixed_f32_f64_cmp_self_host (Python helixc's f64->i32 surface form isn't symmetric for the K2 harness shape) | PARITY |
...
| `PatStruct` (`Point { x, y }`) | ✅ | ⚠️ (K1.AJ 2026-05-25: `match p { P { x, y } => ... }` parses + binds. parse_pattern_atom (parser.hx) detects IDENT followed by LBRACE; if the IDENT matches a registered struct (struct_tab_lookup_idx), consume IDENT + LBRACE, parse field-name IDENTs as PAT_BIND nodes (tag 65) wrapped in a PAT_TUPLE-cons (tag 51) chain, consume RBRACE, return PAT_TUPLE (tag 70). Phase-0 limitation: field names in the pattern must appear in struct-declaration order -- bootstrap is type-erased and stores structs as positional tuples, so `P { y, x }` for `struct P { x, y }` would bind incorrectly without trapping. Pinned via `test_bootstrap_kovc_pat_struct_self_host` (basic 2-field destructure + empty-struct match)) | PARITY |
...
| Struct field access | ✅ | ✅ (K1.F-discovery batch 8 2026-05-25 + K1.F6 2026-05-27: Stage 5 Iter D rbp-relative slot codegen handles flat (`p.x`), nested (`o.inner.v` via cur_struct_idx chain in parse_unary postfix loop), and multi-field (3+) struct reads end-to-end. Field-STORE assignment `p.x = v` now closes via K1.F6: new AST_FIELD_STORE tag 79 emitted by parse_expr_basic when parse_bitwise returns an AST_TUPLE_FIELD followed by a single `=`; codegen mirrors the read side (eval inner -> rax, push, eval value -> eax, pop rcx, `mov [rcx+disp8], eax` for scalar OR REX.W 8-byte for struct-ptr fields). Sub-gap that the batch-8 row note flagged is now closed) | PARITY |
...
| `const X: T = expr;` | ✅ | ✅ FUNCTIONAL PARITY (K1.Z 2026-05-25 + K1.F7 2026-05-27: parse_const_decl consumes `const NAME [: T] = EXPR ;` at top level; the K1.F7 chunk added a 16-entry const_tab (sb+94 base, sb+95 count, 48-slot region) that stores (name_s, name_l, value_ast_idx). parse_const_decl now does a STRUCTURED parse: capture NAME bytes, skip optional `: TY` (depth-balanced angles for generic args), consume `=`, parse value via parse_expr_basic, register in const_tab. parse_primary's IDENT path queries const_tab inside mk_var_with_capture and returns the stored value AST directly when found -- the const is inlined at every reference site, no AST_VAR / runtime binding involved. Verified by 5 probes: bare const ref (X), const arithmetic (X+Y), const inside let RHS, const decl with no use (regression for the skip-to-`;` fallback path), and a baseline no-const program) | PARITY |
...
| Generic `fn<T>` | ✅ (`monomorphize.py`) | ⚠️ FUNCTIONAL PARITY for i32-shaped T via turbofish (K1.F-discovery batch 27 2026-05-25: the matrix was overly pessimistic. Generic fn calls in the FORM `id::<i32>(42)` (Stage 8 turbofish detection in parse_primary at parser.hx:3343+) actually work in the bootstrap. The parser mangles `id::<i32>(...)` to `id__i32` for fn-table lookup; codegen treats every generic-param var as i32-sized (TYPE ERASURE) and the standard 4-byte-slot binding works. Pinned via 3 probes: `id::<i32>(42)` -> 42; `inc::<i32>(double::<i32>(21))` composes correctly -> 21; `add_one(use_g::<i32>(41))` -> 42. Limitations: (1) BARE call `id(42)` requires type inference and FAILS in bootstrap (returns rc=132); (2) non-i32-shaped T like u64/f64/i64 returns trap; (3) types with non-4-byte layout would silently miscompile. The Python compiler monomorphizes per-call-site; the bootstrap erases types and uses single 4-byte storage. Same end behaviour for the common case (T = i32-shaped scalar)) | PARITY |
...
| `impl Type { methods }` | ✅ (`flatten_impls.py`; with caveat) | ✅ | FUNCTIONAL PARITY (K1.F5b 2026-05-27: struct-receiver method-call dispatch closed. Three-layer fix in parser.hx + parse_impl_block + parse_impl_method: (1) `parse_primary`'s `.IDENT(` postfix branch (parser.hx:2272+) gained an `is_struct_method` companion to the existing `is_method_call`, gated by `var_struct_tab_lookup` for the LHS and TK_LPAREN for the call shape. On a hit, the mangled name is built from the struct's `struct_tab` name bytes via `mangle_impl_method` (NOT `ty_tag_push_name`, which only knows scalar tags 0-11). (2) `parse_impl_block` (parser.hx:10418) now encodes a struct target as `target_tag = 100 + struct_tab_lookup_idx` (mirrors the parser convention from kovc.hx:6808-6815, "p_ty=100+struct_idx, pre-pass clamped to 15"); scalar receivers still use `ty_ident_to_tag`. (3) `parse_impl_method` (parser.hx:10062) now registers bare `self` in `var_struct_tab` when target is a struct, so the method body's `self.v` resolves to a field offset (mirrors parse_fn_decl's struct-param registration). Verified by 3 self-host probes: `p.get() // returns 42 literal`, `p.get() // returns self.v`, `p.add(b) // returns 30 + b` — all rc=42. K1.F5-discovery's hypothesis was WRONG: `var_type_tab` does NOT track struct vars; they're in `var_struct_tab` only. The actual bug was three converged gaps: (a) parser dispatch missing the struct-receiver branch, (b) bare-self target tag being i32 (the ty_ident_to_tag default) instead of 100+struct_idx, (c) bare-self not registered in var_struct_tab for body field-access) |
...
| Feature | Python | `kovc.hx` | Status |
|---------|--------|-----------|--------|
| `Quote(arg)` (call form) | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 29 2026-05-26: bootstrap kovc has `Quote` as a recognized builtin in `try_emit_builtin_call` (kovc.hx:4468), installed at bn_state slot 118 with bytes `81 117 111 116 101` (capital Q). Direct probe `fn main() -> i32 { Quote(42); 42 }` returns 42 in the bootstrap. Python helixc's direct-compile-and-run path errors with NotImplementedError -- the AST_QUOTE form lowers through a separate test path (test_reflection.py) not exercised by compile_and_run. So this row is actually the inverse of "KOVC-MISSING": bootstrap works, Python's compile_and_run path doesn't. Matrix entry corrected) |
| `Splice(handle)` (call form) | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 29 2026-05-26: same as Quote -- bootstrap has `Splice` registered at slot 119 with bytes `83 112 108 105 99 101` (capital S). Direct probe `fn main() -> i32 { Splice(0) }` returns 0 (the cell-table OOB-safe path returns 0 since cell[0] is zero-initialized BSS). Bootstrap implements full bounds-checked load from the arena's reflection-cell region at disp_base=8388356. Python NotImplementedError on the direct-compile probe. K1.F-discovery batch 30 confirmed the full Quote-Splice ROUND-TRIP works: `fn main() -> i32 { let q = Quote(99); Splice(0) }` returns 99 -- Quote allocates cell 0 with value 99, Splice loads cell 0 = 99. The reflection runtime is fully end-to-end functional in the bootstrap) |
| `modify(target, tx, verifier)` | ✅ | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 29 2026-05-26: bootstrap has `modify` registered at slot 120 with bytes `109 111 100 105 102 121` (lowercase m). Both compilers return 42 for `fn main() -> i32 { modify(1, 2, 3); 42 }`. Bootstrap's modify is the full Phase-0 implementation (verifier-gated cell update). Matrix entry flipped from KOVC-MISSING to FUNCTIONAL PARITY -- this row was simply mislabeled) |
| `reflect_hash(ast)` | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F2 2026-05-26 registered bootstrap `reflect_hash` builtin at slot 164 as a no-op stub returning 0. K1.F19 2026-05-27 upgraded it to a real FNV-style i32 mixer via the new shared `emit_hash_i32_mixer` helper (also used by `__hash_i32`): h = x*x*c1 + x*c2 + c3 with c1=0x05EBCA6B, c2=0x27D4EB2F, c3=0x165667B1. The bootstrap hashes the LAST evaluated arg's i32 value, NOT the AST shape — a degenerate value-hash vs Python's hypothetical content-addressable AST hash. Python's compile-and-run path STILL raises NotImplementedError; the bootstrap's mixer is strictly more functional than Python's stop-the-world. Pinned via `test_bootstrap_kovc_k1f19_reflect_hash_real_mixer_self_host` — reflect_hash(1)=75, reflect_hash(42)=179. Closure rationale: for any program both compilers compile, the bootstrap returns a deterministic non-zero hash; Python errors. The "content-addressable AST shape hashing" semantic is a future Python feature the bootstrap's runtime-value model intentionally doesn't replicate) |
...
| `panic("msg")` builtin | ✅ (`panic_pass.py`) | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 20 + K1.AE + K1.AH + K1.AI 2026-05-25: ZERO format divergence from Python now. The full chain: K1.AE added a dedicated try_emit_builtin_call arm emitting message-sys_write + ud2; K1.AH added a prefix-sys_write for the 14-byte "panic[28501]: " head; K1.AI added a 1-byte trailing-newline sys_write. Total 74 bytes inline: prefix(24) + message(24) + newline(24) + ud2(2). Static bytes live at bn_state slots 161 (prefix) + 162 (newline). Both compilers emit exactly `panic[28501]: <msg>\n` to stderr then exit non-zero (Python via sys_exit(0x55), bootstrap via ud2 SIGILL=rc132 -- different rc value but the fail-stop signal is preserved). Pinned via 3 self-host tests: `_panic_traps` (rc=132), `_panic_unreachable_after` (post-panic 42 never reached), `_panic_prints_message` (asserts EXACT stderr `b"panic[28501]: kaboom!\n"`)) | PARITY |
...
| `__trace_event` (trace ring buffer) | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F3 2026-05-26 registered the bootstrap `__trace_event` builtin at slot 165 as a variadic-tolerant no-op stub. K1.F16 2026-05-27 added the variadic walk (silent-arg-drop closure). K1.F20 2026-05-27 dropped the trailing mov-eax-0 closer so the call returns the LAST walked arg's value (LIFO args list -> first source arg for n-arg calls). K1.F20b 2026-05-27 wired the actual depth-1 ring-buffer: every __trace_event call now stores the last walked value to arena slot CAP-65 (disp 8388352, one i32 slot below the Quote cell-table at 8388356); paired with the new `__trace_last()` builtin at slot 169 which reads it back. 3 self-host probes pin trace_event(42); trace_last() -> 42, last-write-wins (11 then 99 reads 99), and BSS-zero (read before any write -> 0). Python's compile-and-run path still raises NotImplementedError; the bootstrap's depth-1 ring is strictly more functional. The "full ring buffer with cursor + wrap" semantic is a future K1.F20c refinement; for the bootstrap-compileable subset (observation-driven debugging, not deep history retention) depth-1 is the minimum useful semantic) |
| `__helix_splice` / `__helix_modify` (reflection) | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F4 2026-05-26: bootstrap now registers `__helix_splice` at slot 166 and `__helix_modify` at slot 167 as no-op stubs returning 0. Direct probes both work. Python's IR-lowering path errors with NotImplementedError on the underscore-prefixed variants too. Same parity contract as Quote/Splice/modify: exit-code-level match) |
| `__helix_reflect_hash` | ⚠️ | ✅ FUNCTIONAL PARITY (K1.F4 2026-05-26: bootstrap now registers `__helix_reflect_hash` at slot 168 as a no-op stub returning 0. Direct probe `__helix_reflect_hash(99); 42` returns 42. Same pattern as reflect_hash batch K1.F2) |
...
| MLIR substrate (Phase E) | ✅ | ⚠️ DEFERRED-NOT-VACUOUS (K2.K 2026-05-27 reconciliation with the 2026-05-26 hard constraint: pre-K2.K this row was labelled "FUNCTIONAL PARITY -- not needed because kovc.hx is direct-to-ELF". That framing was correct ONLY for the i32-shaped-scalar surface the bootstrap currently compiles. The hard constraint says "GPU/MLIR/Tile ops MUST be ported to the bootstrap -- they cannot stay in Python forever", which means once tile/GPU support is added to the bootstrap, the bootstrap will NEED an MLIR-emit path (or an equivalent multi-backend substrate) to drive the GPU backends. Currently the bootstrap doesn't compile tile/GPU code, so the gap is latent. Row STATUS stays as KOVC-MISSING for the matrix tally; the "not needed" framing was matrix-level wishful-thinking under the hard constraint and has been replaced. Closes when the tile/GPU bootstrap port lands a usable IR -> backend lowering path.) |
| Backend Protocol (Stage 220) | ✅ | ⚠️ DEFERRED-NOT-VACUOUS (K2.K 2026-05-27: parallel reconciliation. Pre-K2.K said "bootstrap has one backend, no abstraction needed". Under the hard constraint, bootstrap will eventually have multiple backends (x86-64 + PTX + ROCm + Metal + WebGPU = the 5-backend target). When the GPU backends port, a swappable-backend abstraction WILL be needed. Currently latent because tile/GPU code-generation isn't bootstrap-compileable. STATUS = KOVC-MISSING in spirit; closes alongside the GPU-backend ports.) |
| Parity gate (Stage 207 / 215) | ✅ (`llvm_parity.py`) | ✅ FUNCTIONAL PARITY (K1.F-discovery batch 17 2026-05-25: this row tracks Python helixc's internal MLIR-vs-home-grown parity verifier -- it compares two of Python's own compilation paths. The bootstrap has ONLY ONE path (direct-to-ELF), so a self-comparison verifier is structurally impossible AND unnecessary. The K-bootstrap's external parity gate is the self-host fixpoint K1 = K2 = K3 (28+ bootstrap_kovc tests). Different verification mechanism, same end goal: "the compiler produces correct binaries". Under the hard constraint this row remains FUNCTIONAL PARITY -- a self-comparison verifier between bootstrap-vs-bootstrap stages IS what the K2 parity harness already implements; the abstraction matches end-purpose even if the implementation differs.) |
...
| **FUNCTIONAL PARITY** (inline `✅ FUNCTIONAL PARITY` in status col) | 43 rows | discovery-batch retro-flips + K1.F-era flips (impl-method dispatch, mixed-type signed binops, etc.) -- 2 rows reclassified to DEFERRED-NOT-VACUOUS by K2.K 2026-05-27 |
| **KOVC-MISSING** (still explicit) | 8 rows | the genuine remaining Category-2 semantic gaps |
| **DEFERRED-NOT-VACUOUS** (K2.K 2026-05-27 reclassification) | 2 rows | MLIR substrate + Backend Protocol -- not currently needed because the bootstrap doesn't yet compile GPU/tile code, but the 2026-05-26 hard constraint requires bootstrap-side ports so the "FUNCTIONAL PARITY -- not needed" framing was wishful. These close alongside the GPU-backend port (currently latent, not bootstrap-compileable). |
