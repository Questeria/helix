...
| `__trace_event` (trace ring buffer) | ⚠️ | ❌ | KOVC-MISSING (K1.F-discovery batch 28 2026-05-26 RE-AUDIT: the Python side was previously marked ✅ but direct probe via `fn main() -> i32 { __trace_event(); 42 }` raises `NotImplementedError: unknown function '__trace_event' in IR lowering; run typecheck first`. Python is also partially broken -- the IR-lowering pass doesn't recognize the builtin. So this row is actually BOTH-PARTIAL: bootstrap traps via unresolved-CALL ud2, Python NotImplementedErrors. Matrix entry downgraded from ✅ to ⚠️ on the Python side; gap status is unchanged) |
