...
| `reflect_hash(ast)` | ⚠️ | ❌ | KOVC-MISSING (K1.F-discovery batch 28 2026-05-26 RE-AUDIT: same pattern as `__trace_event`. Direct probe `fn main() -> i32 { let h = reflect_hash(1); 42 }` Python raises `NotImplementedError: unknown function 'reflect_hash' in IR lowering; run typecheck first`. Both compilers fail. Python ✅ downgraded to ⚠️. K1.F-discovery batch 29 confirmed bootstrap has no `reflect_hash` slot installed in install_builtin_names -- unlike Quote/Splice/modify which DO exist) |
