...
| **H2** | **Generics — real codegen** | `<T>` **monomorphizes** (no longer type-erased — spec §7 "the most significant gap"). A generics corpus (`Vec<T>`, `Option<T>`, a generic fn over ≥2 differing element types, a generic struct/impl) **compiles AND runs correctly** on the self-hosted compiler, gated by the fixpoint. | 🟡 **IN PROGRESS (2026-06-01)** — monomorphizing (no longer fully erased): generic **functions** over differing scalar types (i32/f32/i64), generic **struct fields** (i32/f32; d.2 `aefea02`), generic **impl methods** at i32 + explicit-return, and **`Option<T>`** (generic enum decl+construct+match via bare `Opt::Some`/`Opt::None`, `e6`→42; `2b2e536`) all compile AND run, fixpoint-gated. **Square-bracket generics** now accepted across **struct/enum/impl** decls (`8c492c8` + `2b2e536` — language consistency + removed two compiler crashes on plausible input). **Remaining for green:** bare-`T` impl-method return at non-i32 (per-instantiation method mono — the key remaining capability gap), a `Vec<T>` container over differing types. **Deferred (non-blocking, bare forms work):** turbofish-on-enum `Opt::<T>::Some` parser hang, f64/i64 8-byte fields. Probe-mapped in `.stage33-logs/v11_state.txt`. |
