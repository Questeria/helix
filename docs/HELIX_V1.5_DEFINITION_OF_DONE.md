...
>
>
> **#4 (Succinct / verifiable-inference receipts) — ✅ DONE (committed LOCALLY, push HELD; 144840f build +
> the audit-PASS refinement commit):** an exact-integer FREIVALDS + FIAT-SHAMIR receipt for the kovc-emitted
> ternary_matmul (C=W·X, the S0 exact-int leg) with an INDEPENDENT host-side checker -- "this run produced
> this output" verifiable WITHOUT re-running the matmul and WITHOUT trusting the runner. All host-side in
> cuda_launch.c (from-scratch FIPS-180-4 SHA-256 [NIST-KAT-gated] + receipt_emit + receipt_check) +
> scripts/gpu_receipt_check.sh; NO new .py (1-.py fence intact), NO kovc edit (fixpoint cdcf8673
> byte-identical). The receipt binds SHA256(W)‖SHA256(X)‖SHA256(C); the checker re-derives the challenges
> r_i = SHA256(H_W‖H_X‖H_C‖i‖j) reduced UNIFORMLY mod p (de-biased) ITSELF (never from the receipt),
> recomputes X·r / W·(X·r) / C·r mod p in O(n^2), asserts W·(X·r)==C·r for t=2 rounds. A range guard |C|<p/2
> makes mod-p equality <=> integer equality, so soundness is EXACT (no tolerance): a forged receipt with
> C≠W·X is accepted with prob <= (1/p)^t = 2^-62 + the SHA-256 collision advantage. O(n^2) << the O(n^3)
> matmul -> FASTER THAN RE-EXECUTION (the checker never runs the kernel). gpu_receipt_check.sh ->
> RECEIPT_CHECK_PASS on the RTX 3070: SHA-256 NIST-KAT + genuine RECEIPT_PASS + SIX NCs each REJECTED by its
> NAMED check (forged-output / kernel-corruption / echo-not-trusted -> CHECK2 Freivalds; tampered-hash ->
> CHECK3; tampered-weight -> CHECK1; vacuity -> fail-closed). gate_ext4 GATE_PASS. **TWO adversarial audits:**
> the design pass (wzayltwpt) chose exact-int Freivalds over the unsound f32-Freivalds; the build audit
> (wxd5788ow) returned PASS (soundness_ok + scope_honesty_ok, no critical/major) and its non-blocking minors
> were then closed -- the challenge was DE-BIASED (rejection-sampled uniform mod p so 2^-62 holds literally,
> not ~2^-60.8) and each NC now asserts WHICH check rejected (pinning the Freivalds-teeth attribution).
> **HONEST SCOPE:** ONE ternary/int matmul output; NOT zero-knowledge (W,X,C revealed to the checker), NOT a
> succinct SNARK (only Freivalds is sublinear), matmul only, one model+input; the f32 SmolLM2/GPT-2
> token-for-token path is the explicitly-deferred SECOND increment (a re-derivable hash-chained transcript --
> Freivalds needs a tolerance over f32 where forgery-soundness is false). A real ZK-SNARK is the post-v1.5 stretch.
>
> **NEXT: #3 (verifiable PTX->SASS) -- the LAST + HARDEST.** S0-S3 + #2 + #4 are DONE; #3 remains as a labeled
> FIRST-increment (the v1.5-complete BAR is below).
...
| **#4** ✅ DONE (144840f+refine) | **Succinct / ZK verifiable-inference receipts** | A receipt format + an independent checker such that "this model on this input produced this output" is verifiable **without re-running the full model and without trusting the runner**, faster than re-execution OR with a re-derivable transcript; checker rejects a forged receipt. | Universal gate + the receipt checker green on a real inference + a forgery negative control. | **DELIVERED:** an exact-integer Freivalds + Fiat-Shamir receipt on the kovc-emitted `ternary_matmul` (C=W·X) + a from-scratch-C independent checker (`receipt_check`); FASTER-THAN-RE-EXECUTION (O(n²) vs O(n³), checker never re-runs the kernel); EXACT soundness ≤ (1/p)^t = 2⁻⁶² (uniform de-biased challenge + a `|C|<p/2` range guard ⇒ mod-p ⟺ ℤ). `gpu_receipt_check.sh` → RECEIPT_CHECK_PASS + 6 NCs each rejected by its named check; gate_ext4 GATE_PASS (fixpoint cdcf8673, 1-.py fence intact); two audits (design wzayltwpt + build wxd5788ow PASS). **Scope (honest):** ONE ternary/int matmul output; NOT zero-knowledge, NOT a succinct SNARK, matmul-only, one model+input; the f32 SmolLM2 token-for-token path = the deferred SECOND increment (re-derivable transcript); full ZK = post-v1.5 stretch. |
| **#3** ✅ DONE (2dc24b3…66a6e68) | **Verifiable PTX→SASS (kill the last trusted closed binary)** | A from-scratch, verifiable PTX→SASS path (or verified SASS validator) for the emitted `sm_86` kernel subset, so the hex0→…→PTX→**SASS** chain has no trusted closed `ptxas` in it for that subset; output verified against a reference. | Universal gate + the SASS path/validator green on the kernel subset, with a negative control. | **DELIVERED:** a from-scratch SASS→spec **TRANSLATION-VALIDATION** for the `vector_add` sm_86 subset that removes ptxas's PTX→SASS lowering from the TCB. The de-trust **VERDICT is the COMPOSITE `sass_tv` AND `sass_exec`**: `sass_tv` (from-scratch CPU — ELF `.text` decode + symbolic structural all-inputs proof: data-independence + straight-line-enforced + unique-store + modifier-/field-complete + tag-invalidating + a load-bearing interpreter cross-check, proving `c[gid]=a[gid]+b[gid]` for ALL f32) **AND** `sass_exec` (GPU-differential — the candidate's real RTX-3070 execution == the from-scratch interpreter, discharging instruction-scheduling/scoreboard correctness, input-independent). `scripts/gpu_sass_tv_check.sh` → **SASS_TV_CHECK_PASS** with **10 NCs** (a wrong kernel — modifier / width / control-flow / double-store / stale-tag / scheduling — REJECTED). Hardened through **5 adversarial audits** (each found+fixed real P0/P1 until a clean pass: wtmv9bcog→w9hjc67k4→wksw71qo0→wdxvu1koz→wmsmnfu8y-PASS). NO kovc edit (fixpoint cdcf8673), host-side. **Scope (honest):** ptxas removed from the TCB for **ONE** kernel (`vector_add`), f32, sm_86/CUDA-12.8-pinned; residual = ptxas-ran + GPU/driver/silicon execute the validated SASS **AND** the GPU is the scheduling differential-oracle; a translation-validation **WITNESS** (per-compilation, machine-checked), **NOT** a formal proof of ptxas. A pure-CPU scoreboard model + multi-kernel/arch coverage are labeled multi-week+ stretches. |
