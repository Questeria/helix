...
- **Committed C/H after V6: 24 files, 15 600 LOC** (was 30 files / 16 308 LOC; V6 pruned 6 dead
  files / 708 LOC — see §4).
- The trusted-C surface is **two disjoint categories**:
  - **A. The from-raw bootstrap ladder** (`stage0/*`, 22 files / 13 217 LOC) — trusted **source**
    ...
    self-host fixpoint.
  - **B. The runtime/GPU harness** (`helixc/runtime/`, 2 files / 2 383 LOC) — `cuda_launch.c`
    (1923) + `train_transformer.c` (460). **Outside** the self-host fixpoint. The genuinely
